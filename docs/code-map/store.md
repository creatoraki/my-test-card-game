...
| [townStore.ts](../../src/store/townStore.ts) | 持久化城镇档案：角色 profile、回城记录的当前 HP、唤醒、编队、经验池、个人卡组、污染值、生病、怪癖、卡组锻造、免费三选一/删卡、居民积分、仓库、装备和出售，以及生存天数 `day`、商店货架 `shop` 与 `squadTalent`（`badgeId` + 已激活节点 id 数组 `nodes`，`activateTalentNode` 点亮 / `refundTalentNode` 单点退还，判定走 `data/squadTalents`）。训练点由上阵角色卡组等级之和实时计算，存档 key 为 `town-profile-v10`。 |
