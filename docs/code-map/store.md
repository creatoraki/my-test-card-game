...
| 文件 | 作用 |
| --- | --- |
| [battleStore.ts](../../src/store/battleStore.ts) | 单场战斗状态包装。`play/end` 前先 `structuredClone`，再调用引擎，避免 React 持有对象被原地修改；`seq` 标识新战斗，供 UI 重置分镜。`BattleMeta` 只承载羁绊，挑战词条随 `BattleState` 逐帧提交。 |
