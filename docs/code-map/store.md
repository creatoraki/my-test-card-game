...
| store | 生命周期 | 作用 |
| --- | --- | --- |
| [townStore.ts](../../src/store/townStore.ts) | **持久化**（localStorage，键名 `town-profile-v28`，版本号 30） | 城镇档案，存放跨远征的永久资产：角色卡组、装备、经验、居民积分、仓库、编队、阵亡名单、小队天赋、医疗室、图鉴、地图进度、商店与科技。本文件只负责拼装切片、建档 / 重置与「推进一日」。 |
| [runStore.ts](../../src/store/runStore.ts) | 内存 | 一次远征的流程编排：界面路由 `screen`、进出探索、建战斗、战后结算、撤离与回城。单向依赖 `townStore`。本文件只保留状态与路由，编排细节在 `store/run/`。 |
| [exploreStore.ts](../../src/store/exploreStore.ts) | 内存（出击前会备份城镇档案） | 包裹 `explore/session/` 的远征会话本身。 |
...
| 文件 | 作用 |
| --- | --- |
| [townTypes.ts](../../src/store/townTypes.ts) / [townProfile.ts](../../src/store/townProfile.ts) | 城镇档案的状态形状；出厂档案（建档、重置、复苏共用）与训练点计算。 |
| [deckForgeSlice.ts](../../src/store/deckForgeSlice.ts) | 卡组锻造：升级、抽卡、删卡、降低最小卡组下限、装备羁绊重铸。 |
| [storageSlice.ts](../../src/store/storageSlice.ts) | 仓库与装备：落袋、出售、换金物变现、穿脱装备、卡牌模组装配、模组制造。 |
| [memberCareSlice.ts](../../src/store/memberCareSlice.ts) | 队员生命周期：上下阵、阵亡与复苏、营养舱、圣水池。 |
| [conditionSlice.ts](../../src/store/conditionSlice.ts) | 经验、污染与怪癖：发经验、污染 / 净化卡牌、战斗与回城时的状态回填。 |
| [squadTalentSlice.ts](../../src/store/squadTalentSlice.ts) | 小队天赋：徽章选择与节点点亮 / 退还。 |
