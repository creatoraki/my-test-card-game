...
| [transitions.ts](../../src/ui/app/shared/transitions.ts) | 过场动效预设表，也是时长常量的唯一来源；视觉部分在 `ScreenTransition.module.css`。 |
| [transitionOrigin.ts](../../src/ui/app/shared/transitionOrigin.ts) | 记录触发过场的点击坐标，只用一次，读取后立即清空。不进 store。 |
...
| [installGameCursor.ts](../../src/ui/app/shared/installGameCursor.ts) | 全局光标：预加载三种状态，左键按下的反馈至少显示 120 毫秒。 |
...
| 文件 | 作用 |
| --- | --- |
| [assetLoader.ts](../../src/ui/art/loader/assetLoader.ts) / [assetPreloader.ts](../../src/ui/art/loader/assetPreloader.ts) | 素材预加载：图片限 2 个并发并利用空闲时间加载，视频只等首帧；对外提供预加载进度快照。⚠ 要预加载的 URL 必须登记在下面的表中。 |
| [sceneArt.ts](../../src/ui/art/explore/sceneArt.ts) | 主菜单背景视频、电梯视频、标题图。 |
| [cardArt.ts](../../src/ui/art/battle/cardArt.ts) / [enemyArt.ts](../../src/ui/art/battle/enemyArt.ts) / [itemArt.tsx](../../src/ui/art/items/itemArt.tsx) | 卡牌大图、敌人立绘（附带用 `alpha-bbox.mjs` 测出的内容框）、物品图标（没有美术的物品回退到内联线框 SVG）。 |
| [statusArt.ts](../../src/ui/art/battle/statusArt.ts) / [statusAccent.ts](../../src/ui/art/battle/statusAccent.ts) / [buffArt.ts](../../src/ui/art/battle/buffArt.ts) | 战斗状态图标及其主题色；非战斗状态的徽记（培育、组装部件、药剂）。 |
| [battleBg.ts](../../src/ui/art/battle/battleBg.ts) / [mapArt.ts](../../src/ui/art/explore/mapArt.ts) | 按地图 id 登记的战斗背景和地图预览图。 |
| [corridorArt.ts](../../src/ui/art/corridor/corridorArt.ts) / [corridorPlayerArt.ts](../../src/ui/art/corridor/corridorPlayerArt.ts) / [corridorPlayerFrames.ts](../../src/ui/art/corridor/corridorPlayerFrames.ts) | 探索场景的道具缩放和素材、探索角色的逐帧动画。 |
| `ecoArk*.ts` | 生态方舟的建筑、敌人、道具和近景素材。 |
| [eventDossierArt.ts](../../src/ui/art/explore/eventDossierArt.ts) / [eventArt.ts](../../src/ui/art/explore/eventArt.ts) | 事件档案插图。⚠ `eventArt` 按旧节点事件类型登记，目前全部是占位图。 |
| [moduleGlyphs.tsx](../../src/ui/art/moduleGlyphs/moduleGlyphs.tsx) + `moduleGlyphs*.tsx` | 每件模组专属的 SVG 徽记，按精算师、炼金术士、1 阶通用分文件维护。 |
| [shopArt.ts](../../src/ui/art/town/shopArt.ts) / [rarityArt.ts](../../src/ui/art/items/rarityArt.ts) / [techTreeArt.ts](../../src/ui/art/town/techTreeArt.ts) | 商店货位边框、稀有度水晶、科技节点的金属纹样。 |
...
| 文件 | 作用 |
| --- | --- |
| [stage.ts](../../src/ui/app/shared/stage.ts) | 设计画布恒为 1920×1080，由 `--stage-scale` 等比缩放。所有 px 都是设计 px。 |
