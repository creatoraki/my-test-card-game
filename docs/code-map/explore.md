...
| `corridor/types.ts` | 房间内的位置、朝向、物件、传送门、BOSS 红门、遭遇状态与场景常量；房间宽度读取近景 1.7 倍显示尺寸，槽位按该宽度换算。 |
