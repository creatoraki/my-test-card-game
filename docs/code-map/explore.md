...
| `corridor/session.ts` | 单个房间的场景展开（`buildRoomScene`）、交互距离、传送门与红门判定、面板锁定、墙体夹取、物件开启/取消和战后状态。 |
| `corridor/ambush.ts` | 暗雷的累计粒子消耗、线性遭遇概率、玩家身前的生成位置、杂兵档位加权与暗雷节点追加；纯探索状态逻辑。 |
| `energy.ts` | 净化粒子的唯一改写口，会话与地牢模块共用。 |
| `../data/curios/` | 11 种采纳物件、安全投递柜、流浪货商、机械小生物双食品、奖励池与货商食品池/价值分档；每个货商只收两种临期食品，守卫事件按房间深度抽档，暗雷事件按 t1/t2 权重生成。 |
