...
当前远征使用侧视横向走廊，每层 8 个可跳过的物件、中途黑影和终点守卫。`generateRound` 根据地图难度确定战斗档位后交给 `corridor/session.ts` 创建本层。普通远征默认 6 层，新手地图沿用自身层数，最后一层为首领。原路由生成器另名为 `generateRouteRound` 保留，已不参与当前探索入口；下文关于桥接、限时揭示、旧事件池的描述属于保留模块。

横向探索的阶段链为 `atNode → landed → resolving → atNode`；暂不处理物件可直接回到 `atNode` 且不扣粒子。敌人链为 `atNode → encounter → inBattle`：黑影演出完成后，中途遭遇复用节点战斗，胜利返回原地并消除黑影；终点遭遇复用轮次战斗，胜利生成下一层或通关。领取奖励和背包规则继续由原会话结算处理。

| 横向探索模块 | 职责 |
| --- | --- |
| `corridor/types.ts` | 位置、朝向、物件、遭遇状态和移动常量。 |
| `corridor/session.ts` | 本层生成、交互距离、敌人阻挡、物件开启/取消和战后状态。 |
| `../data/corridorCurios.ts` | 8 种物件的选项、效果和图集格位。 |
| `../store/exploreCorridor.ts` | 保存位置、打开物件、遭遇演出结束后的原子战斗编排。 |
