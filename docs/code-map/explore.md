...
**房间制（没有「层」）**：一张地图就是一张房间网格图，房间总数 `MapDef.roomCount` 就是这张图的庞大程度。`generateDungeonRun` 在建局时生成整张图并落到起始房间，之后整趟远征不再重新生成。每间房宽 3840、高 1080，镜头在 1920×1080 的可视区域内跟随玩家并在两端夹紧；房间最多连通上下左右 4 个方向（最多 4 座传送门，四座外观完全一致，方向只能从小地图读出）。传送门优先占据左右边缘，额外的门与可交互物共用中段槽位。粒子口径只有三项：**每移动 1 个房间 −5、每交互 1 个事件 −2、每进行 1 个战斗回合 −1**。
...
| 房间制模块 | 职责 |
| --- | --- |
| `dungeon/types.ts` | 房间节点、近景地图变体、出口方向、房间图状态与方向换算。 |
| `dungeon/generate.ts` | 网格房间图生成：生成树 + 少量环路、BFS 深度、BOSS 房与战斗房投放、传送门边缘优先及中段槽位分配；新手关卡按会话 RNG 为房间混排两种近景地图并保证两种都出现。全程同种子复现。 |
| `dungeon/session.ts` | 进入房间、站上传送门点亮目标、确认传送扣粒子、房间探索完成判定、场景进度回写房间图。 |
| `corridor/types.ts` | 房间内的位置、朝向、物件、传送门、遭遇状态与场景常量（含房间宽度、可视宽度、边缘及中段槽位表）。 |
