...
| `dungeon/curioPlan.ts` | 随机地图每房一至两个物件；起点固定临时祝福匣，陷阱房固定风险物件，非陷阱房按 `healChance` 抽治疗物件。其余名额把通用权重与地图 `curioPool.weights` 合并后不放回抽取；治疗／风险池允许地图覆写，货商占一个名额并排在最后。 |
| `dungeon/nearMapGeometry.ts` | 近景原始宽高与统一 1.7 倍缩放。地图可通过 `nearMapVariant` 在布局前指定房间几何；方舟使用 2171×724 的独立平台，宽度、传送门、物件槽位和镜头边界保持一致。 |
...
| `energyCost.ts` | 净化粒子计价的唯一口径：`interactionCost`（按物件分类分档）、`roomMoveCost` / `roomMoveCostFor`（新房/回头路）、`spendBattleEnergy`（回合 + 档位）、`spendWalkEnergy`（行走）。session.ts 转出前两者供旧调用方使用。 |
| `relicModifiers.ts` | 探索级遗物的持有判定与数值修正：负重适应、货商额外货位、废料回收加成和应急信标可用性。 |
| `../data/curios/` | 随机物件（搜刮/补给/改装/道具奖励 `lootCurios.ts`）、3 种风险房物件（`riskCurios.ts`，`forced` 进房即触发）、安全投递柜、流浪货商与 5 种新手专属物件；物件以 `role` 标记物品/治疗/风险/服务、机械小生物双食品、奖励池与货商食品池/价值分档；每个货商只收两种临期食品，随机地图守卫按房间深度抽档，蓝图守卫使用固定遭遇，暗雷事件按 t1/t2 权重生成。 |
