...
| `dungeon/generate.ts` | 调用紧凑房间生成，计算 BFS 深度、BOSS/战斗房投放，以及按门数规则分布传送门并过滤/兜底传送门附近的中段槽位；从 `RANDOM_CURIO_KINDS` 投放普通物件并按地图大小追加 1–2 个流浪货商。检测到地图蓝图时转交 `planned.ts`。全程同种子复现。 |
