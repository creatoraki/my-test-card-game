...
设施升级的公共视觉实现集中在 `common/techTree/Technology*`：`TechnologyTree` 提供纯净背景和页头比例，`TechnologyGraph` 持有连线与节点布局，`TechnologyMedallion` 绘制圆环、六边框和状态角标，`TechnologyDetail`、`TechnologyMaterials` 与 `TechnologyFooter` 分别负责右栏、材料及底栏。各模块仅导入同名样式。商店私有 `MarketUpgradePanel.module.css` 只负责画布内浮层与开关动画。
