...
| [town/training/TalentArtwork](../../src/ui/town/training/TalentArtwork/) | 天赋页的独立视觉组件：`TalentPanelShell` 将 1672×941 原型坐标缩放到编队画布中央的 1536×864 面板，背景收进外框边界；`TalentBorder`、`TalentEmblem`、`TalentPlaque`、`TalentNode` 和 `TalentResourceFrame` 以 SVG 绘制金属外框、星芒徽记、分色铭牌、状态圆环及通栏装饰，标题与悬浮提示各自模块化。`BadgeRim` 持有三枚小队徽章共用的外框骨架（外发丝 r=92 → 底盘 r=87 → 虚线环 r=79 → 内盘 r=70 → 内细环 r=62）与三档配色，四向尖顶饰和刻度环的花样按徽章分档放在 `BadgeRimMotifs`（启程罗盘、先手箭簇、守时齿牙）；它不带 `<svg>` 根，约定父级 viewBox 为 `-100 -100 200 200` 且根节点 `fill="none"`，`TalentEmblem` 与 `BadgeCoreArtwork` 共用同一份。 |
| [town/training/BadgeSelectModal](../../src/ui/town/training/BadgeSelectModal/BadgeSelectModal.tsx) | 天赋页左侧的 510×864 黑金悬浮选择面板，无二次全屏遮罩。`BadgePanelFrame` 绘制金属框与卡片花角，`BadgeScrollRail` 联动真实列表滚动；`BadgeCoreArtwork` 为启程直接复用天赋中央 `TalentEmblem`，先手与守时分别采用红色爆发、蓝色循环图案；三枚共用 `BadgeRim` 的金属外框骨架，配色与尖顶/刻度花样按徽章分档（启程金色罗盘、先手赤铜箭簇、守时寒钢齿牙）。保留现有徽章名称、基础效果、切换确认及退点提示，支持上下方向键与首尾键浏览。 |
...
| [explore/ExploreScreen](../../src/ui/explore/ExploreScreen/ExploreScreen.tsx) | 房间探索主界面，只编排固定设计画布、房间场景、小地图、净化粒子、底部 HUD、奖励和撤离。场景以 `corridor.roomId` 为 key，换房间即整屏重建；`usePortalTravelTransition` 在黑场淡入完成后提交传送，再淡出揭开新房间。`ExploreDock` 承载底部整条 HUD；`ExploreInventory` 与 `useExploreInventory` 管理负重读数、用药目标、角色装备、野餐和应急信标选点；`CurioPanel` 通过 `EventDossierPanel` 展示奇物的行动/放入/结算界面，`WanderingMerchantPanel` 展示持续货架。物件奖励复用 `RewardOverlay` / `LootPickup`，战斗仍由 `runStore.enterEncounter` 建局。 |
| [explore/EventDossier](../../src/ui/explore/EventDossier/EventDossierPanel.tsx) | 探索物件事件的「事件档案」浮层：1336×904 折角外框(几何见 explorePanel 的 panel-box)、右侧事件插图、左栏页眉/大标题/英文副标题、装饰层与 ✕ 关闭。场景组件 `DossierChoice`(行动选择)、`DossierResult`(结算)、`DossierOffer`(放入物品)共用阶段行、信息框与右下两列按钮网格 `DossierActionGrid`；插图映射在 `dossierArt.ts`。 |
...
| [explore/ExploreScreen/BossGatePanel](../../src/ui/explore/ExploreScreen/BossGatePanel.tsx) | BOSS 红门事件面板：复用 `EventDossierPanel` 与 `DossierChoice`，以红色 accent 展示挑战选项；挑战时记录点击坐标，驱动裂纹转场从按钮位置开始，焦点与 Esc 由公共 hook 管理。 |
...
| [fx/TwinArrowFx](../../src/ui/battle/fx/TwinArrowFx/TwinArrowFx.tsx) | `twin-arrow` 画布中心锚点 Canvas 特效：两段命中，弓位固定在目标左下；震屏归 `SHOTS.twin`，全屏闪归 `screenFx: "twin"`；`test/opus` 下的 demo 直接引用本件，不保留副本。 |
...
生态方舟的素材分为 `ecoArkEnemyArt.ts`（敌人立绘）、`ecoArkPropArt.ts`（交互物尺寸与地面留白）、`ecoArkScenery.ts`（探索远近景）和 `ecoArkArchitectureArt.ts`（近景建筑装饰）。它们合并进既有素材表与启动预加载清单。选图使用现有生态方舟配图，战斗背景单独登记；`CorridorFar` 按地图切换远景，`EcoArkArchitecture` 按房间编号稳定随机建筑位置，并避开传送门和交互物。
...
| [art/corridorArt.ts](../../src/ui/art/corridorArt.ts) | 废弃楼层走廊背景与道具素材映射、预载来源登记；房间传送门使用蓝色立绘，交互物按类型使用独立透明 PNG，并登记小/中/大三档缩放、底部透明留白与场景 Y 轴偏移。 |
