...
| [town/training/BadgeSelectModal](../../src/ui/town/training/BadgeSelectModal/BadgeSelectModal.tsx) | 天赋页左侧的 510×864 黑金悬浮选择面板，无二次全屏遮罩。`BadgePanelFrame` 绘制金属框与卡片花角，`BadgeScrollRail` 联动真实列表滚动；`BadgeCoreArtwork` 为启程直接复用天赋中央 `TalentEmblem`，先手与守时分别采用红色爆发、蓝色循环图案。保留现有徽章名称、基础效果、切换确认及退点提示，支持上下方向键与首尾键浏览。 |
