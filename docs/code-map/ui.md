...
| [GuideSpotlight](../../src/ui/common/GuideSpotlight/GuideSpotlight.tsx) | 全局聚光灯模态引导：从 `guideStore` 消费队列，按 `data-guide-anchor` 实时测量并 portal 到设计画布；高亮目标时挖出遮罩，找不到目标则居中提示，Enter / Esc / 「知道了」关闭。`useGuideOnce` 把已看过的引导 id 写入 `townStore.seenGuides`，用于新手奖励操作的一次性提示。 |
