...
| [explore/CorridorScene](../../src/ui/explore/CorridorScene/CorridorScene.tsx) | 严格横向行走、镜头跟随、交互目标和行进图。`useCorridorMovement` 管理方向键与字母方向键、触摸按钮、失焦暂停和位置保存；`CorridorSprite` 读取生成的九宫格图集；`CorridorPlayer` 从六帧横向精灵图播放行走动作并以第五帧站立；`ShadowEncounter` 播放黑影破地动画并将战斗转场圆心定位到黑影。 |
