...
| [character/CharacterDetailView/FigureStage](../../src/ui/character/CharacterDetailView/FigureStage/FigureStage.tsx) | 详情态左栏：76,196,434,772 的蓝紫切角立绘窗；复用宿舍静态背景与角色立绘，保持飞行层取景。`FigureFrame` 用无滤镜 SVG 叠线绘制边框，`figureCopy` 管理中文档案文案，`FigureProgress` 展示真实卡组等级、经验与升级提醒；升级按钮直接打开锻造层的 `upgrade` 面板。底栏保留当前小队徽章、三段生命、污染、怪癖，血条以 `animated={false}` 关闭装饰动画；悬浮卡牌时用透明度遮罩压暗。版面矩形由 `morphChoreo.FIGURE_RECT` 统一下发。 |
