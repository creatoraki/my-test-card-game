...
| [explore/ExploreScreen](../../src/ui/explore/ExploreScreen/ExploreScreen.tsx) | 横向探索主界面，只编排固定设计画布、走廊场景、净化粒子、队伍、奖励和撤离。`ExploreInventory` 与 `useExploreInventory` 管理背包、用药目标、角色装备和野餐；`CurioPanel` 展示物件选项与结算。物件奖励复用 `RewardOverlay` / `LootPickup`，战斗仍由 `runStore.enterEncounter` 建局。 |
| [explore/CorridorScene](../../src/ui/explore/CorridorScene/CorridorScene.tsx) | 严格横向行走、镜头跟随、交互目标和行进图。`useCorridorMovement` 管理方向键与字母方向键、触摸按钮、失焦暂停和位置保存；`CorridorSprite` 读取生成的九宫格图集；`ShadowEncounter` 播放黑影破地动画并将战斗转场圆心定位到黑影。 |
