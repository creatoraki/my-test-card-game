...
路径：`src/ui/battle/`。战斗画布固定为 1920×1080。战斗状态由 `store/battle/battleStore` 提供；引擎把每次操作的结果记成 `steps`，UI 按顺序回放。
...
| 目录或文件 | 作用 |
| --- | --- |
| [unitShell.ts](../../src/ui/battle/choreo/unitShell.ts) | “战斗单位外壳”的跨组件约定：敌人的 `CombatantView` 和我方的 `AllyBar` 共用。 |
| [CombatantView/](../../src/ui/battle/CombatantView/CombatantView.tsx) / [EnemySprite/](../../src/ui/common/unit/EnemySprite/EnemySprite.tsx) | 场上的敌人：立绘、血条、状态、意图、命中率徽章；待机动画由纯 CSS 横向拼条循环播放。 |
| [AllyBar/](../../src/ui/battle/AllyBar/AllyBar.tsx) | 我方队伍的玻璃头像卡。 |
| [HandTray/](../../src/ui/battle/HandTray/HandTray.tsx) / [HandCard/](../../src/ui/common/card/HandCard/HandCard.tsx) / [HandTools/](../../src/ui/battle/HandTools/HandTools.tsx) | 手牌托盘、单张手牌（`parts/` 中有印记、模组标记、污染裂纹）、弃牌 / 重抽 / 等待工具。 |
| [handFocusStore.ts](../../src/ui/battle/state/handFocusStore.ts) | 手牌悬停状态的极小 store。单独抽出是为了性能：悬停变化不会让整个战斗页重新渲染。 |
...
| [CardPile/](../../src/ui/battle/CardPile/CardPile.tsx) / [PileRail/](../../src/ui/battle/rails/PileRail/PileRail.tsx) / [PileDrawer/](../../src/ui/battle/PileDrawer/PileDrawer.tsx) | 牌堆、牌堆入口栏、牌堆抽屉（同时支持“从牌堆挑一张”的选择模式）。 |
| [RelicRail/](../../src/ui/battle/rails/RelicRail/RelicRail.tsx) / [BondRail/](../../src/ui/battle/rails/BondRail/BondRail.tsx) / [ChallengeRail/](../../src/ui/battle/rails/ChallengeRail/ChallengeRail.tsx) | 遗物栏、羁绊栏、挑战词条栏。 |
...
| [BattleToast/](../../src/ui/battle/BattleToast/BattleToast.tsx) + [battleToastStore.ts](../../src/ui/battle/state/battleToastStore.ts) | 战斗内的轻提示。 |
| [SkillCutInCard/](../../src/ui/battle/SkillCutInCard/SkillCutInCard.tsx) | 出牌时的卡面亮相。 |
| [AmbienceLayer/](../../src/ui/battle/AmbienceLayer/AmbienceLayer.tsx) + [ambience.ts](../../src/ui/battle/AmbienceLayer/ambience.ts) | 用 Canvas 绘制的场景粒子，按地图 id 选择预设。 |

## 演出预设（时长的唯一来源）

| 文件 | 作用 |
| --- | --- |
| [animations.ts](../../src/ui/battle/choreo/animations.ts) | 出牌动画预设：分类、首击特效、主色、时间轴参数。被 41 个文件引用。 |
| [animSfx.ts](../../src/ui/battle/choreo/animSfx.ts) | 动画和音效的时序对齐表。 |
| [hitFloats.ts](../../src/ui/battle/choreo/hitFloats.ts) | 把引擎的逐段命中明细展开成飘字序列和命中特效。 |
| [deathChoreo.ts](../../src/ui/battle/choreo/deathChoreo.ts) / [victoryChoreo.ts](../../src/ui/battle/choreo/victoryChoreo.ts) | 死亡消散的等待门槛；胜利面板的演出节奏。 |
...
[VictoryPanel/](../../src/ui/battle/victory/VictoryPanel/VictoryPanel.tsx) 是主面板，由以下部分组成：`VictoryExpRow`（经验）、`VictoryDropSection`（能量档位与掉落）、`VictoryBoonTray`（治疗露珠、卡牌、装备箱）、`VictoryCardOffer`（卡牌候选）、`VictoryLootTray`（战利品盘）、`VictoryBackpack`、`VictoryPlaque` 和 `VictoryButton`。⚠ `VictoryPanel.tsx` 中丢弃物品时仍在用 `window.confirm`。
