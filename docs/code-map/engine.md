...
| [targeting.ts](../../src/engine/targeting.ts) | 存活单位、敌我查询和目标合法性判定。`tauntedAmong` / `validFoeTargetIds` 统一维护嘲讽筛选；普通敌人优先在存活的嘲讽目标中等概率随机选取，没有嘲讽时从全部存活单位中随机选取；玩家单体敌方卡、弃牌自动目标和战场点选也复用同一判定，群攻不受限。脚本敌人的强制目标由 `enemyScript.ts` 在玩家嘲讽不存在时保持优先。 |
...
| [ai.ts](../../src/engine/ai.ts) | 敌人按招式权重抽招与行动执行：脚本敌人经 `enemyScript.ts` 按护盾状态和 AI 记忆选招，普通敌人经 `enemyScript.ts` 计算 `bias` 条件权重后随机抽招；按招式延迟开始蓄力、倍率预览、将招式级命中修正注入 DAMAGE 效果、行动前推进状态节拍(`runEnemyTempoPhase` 拆出, 供 `actAndRecord` 把 DOT/HOT 单独录成一帧播在出招之前)、眩晕跳过、随机或脚本目标选择和效果解释。每回合行动点在开始时补满，招式发动后按剩余行动点继续选招，用尽后 `nextActTick = null`。 |
| [enemyScript.ts](../../src/engine/enemyScript.ts) | 可序列化敌人行动脚本的纯逻辑：按我方护盾状态、后继权重和回收/喘息/重锤约束选择招式，更新行动记忆；普通小怪的 `bias` 条件判定、`targetPick` 的最高护盾/治疗空间/带状态/不带状态筛选也集中在此。玩家存在嘲讽时，脚本目标选择让位给公共嘲讽判定。 |
