...
| [ops.ts](../../src/engine/ops.ts) | 伤害、治疗、护盾、施加状态、战斗内属性修正、弃牌回调、状态生命周期和胜负判定等原语；状态统一支持持续回合扣减与清理，并提供体力极限恢复和护盾击破钩子；我方 0 血进入濒死，再受伤按 `RULES.combat.downedDeathChance` 掷死亡骰；死亡时清空该角色抽牌堆/手牌/弃牌堆卡牌。护盾在战斗内跨回合保留，仅随战斗结束消失。敌人死亡和实际 HP 伤害在这里接入挑战判定。伤害顺序固定为状态修正 → 命中 → 暴击 → 防御 → 格挡 → 护盾 → HP → 荆棘；固定伤害跳过防御与格挡。 |
...
| [statuses.ts](../../src/engine/statuses.ts) | 中毒、灼烧、再生、星辉、铁壁、力量、粒子污染、锋利、不周山、虚弱、易伤、荆棘、眩晕、洞察、瞄准、龙舌兰、充能外壳等状态注册表。再生按层固定回复并由 `duration` 过期，铁壁按层提供防御，龙舌兰通过状态百分比修正提高攻击，充能外壳提高攻击并在护盾击破时触发眩晕、临时卡和 AI 记忆反馈。粒子污染永久可叠层，按层提高攻击伤害，闪避由 `stats.ts` 派生并封顶。状态通过 `ctx.ops` 调用原语，避免直接依赖引擎实现造成循环依赖。眩晕对我方按回合扣层并由出牌入口封牌，对敌人按行动次数扣层并跳过行动，洞察的实际处理在 UI。 |
