...
| [statuses/](../../src/engine/statuses/) | 状态定义分表：`dot.ts` 负责持续伤害/治疗、反伤与生机培育加速, `abandonedFloor.ts` 负责静电、易燃、焦灼、回收装甲、护航与导电薄膜, `botanist.ts` 负责双生花、根系联结、棘冠、常春藤刺与锚定瞄准, `buffs.ts` 负责通用增益与锋利增伤池, `swordsman.ts` 负责镜月、铁衣、风切、残心、残心·凝神与八千代, 其余文件负责保险、减益、控制与叠加策略；`index.ts` 合并并提供注册表。 |
