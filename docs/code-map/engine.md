...
路径：`src/engine/`。纯 TypeScript 的战斗规则层，不依赖 React 和 store。UI 和 store 只能通过 [index.ts](../../src/engine/index.ts) 引入。所有函数都会直接修改传入的 `BattleState`，由 `store/battleStore` 先克隆再调用。
...
| 文件 | 作用 |
| --- | --- |
| [battle.ts](../../src/engine/battle.ts) | 对外的高层操作：`createBattle`、`startRound`、`canPlay` / `playBlockReason`、`playCard`、`discardHandCard`、`redrawHandCard`、`waitTick`、`endRound`。 |
| [battleSetup.ts](../../src/engine/battleSetup.ts) | 根据遭遇战和队伍快照建局，注入敌人的基础命中和格挡。⚠ 直接读取 `import.meta.env.BattleTest`，开启时敌人只有 1 点生命。 |
| [scheduler.ts](../../src/engine/scheduler.ts) | 时刻调度（本作核心）：打出普通牌推进 1 个时刻并结算到点的敌人行动；速攻牌不推进时刻。 |
| [statusLifecycle.ts](../../src/engine/statusLifecycle.ts) | 按时刻和敌我阵营运行状态的持续与结算。 |
| [battleChoices.ts](../../src/engine/battleChoices.ts) | 战斗中需要玩家做选择的结算与取消（例如从抽牌堆挑一张）。 |
| [flee.ts](../../src/engine/flee.ts) | 敌人到点离场（宝箱怪）。 |
...
| [cardFx.ts](../../src/engine/cardFx.ts) | 弃牌联动与被动卡的演出记录器。单独成文件是为了打破 `discard` ↔ `passive` 的循环依赖。 |
| [passive.ts](../../src/engine/passive.ts) / [passiveCards.ts](../../src/engine/passiveCards.ts) | 被动卡：没有费用、不能打出，持在手中时按事件自动结算，回合结束时收入弃牌堆。 |
| [keywords.ts](../../src/engine/keywords.ts) | 卡牌词条注册表，负责词条的判定和触发后的副作用。 |
| [cardMarks.ts](../../src/engine/cardMarks.ts) / [cardEffects.ts](../../src/engine/cardEffects.ts) | 卡面印记的定义；卡牌的基础效果和当前生效效果（已叠加印记与模组）。 |
| [cardBoon.ts](../../src/engine/cardBoon.ts) | 判定手牌是否处于“激活态”（培育成熟、降费、星光支付、共鸣等条件已满足），供手牌高亮使用。 |
| [cardText.ts](../../src/engine/cardText.ts) | 渲染卡牌说明文字（把 `{0}` 等占位符替换成实际数值）。 |
| [cost.ts](../../src/engine/cost.ts) | 法力与星光费用的计算和支付。 |
| [cultivate.ts](../../src/engine/cultivate.ts) / [waterfall.ts](../../src/engine/waterfall.ts) / [fullDraw.ts](../../src/engine/fullDraw.ts) / [handChoice.ts](../../src/engine/handChoice.ts) | 角色专属机制：培育阶段、瀑布再演、`fullDraw` 效果的命中结算（与穿刺层数联动）、弃牌选择的数量计算。 |
| [counters.ts](../../src/engine/counters.ts) / [pierce.ts](../../src/engine/pierce.ts) / [insurance.ts](../../src/engine/insurance.ts) | 计数器、穿刺层数、精算师的保险层数。 |
...
| [damage/preview.ts](../../src/engine/damage/preview.ts) / [hitPreview.ts](../../src/engine/hitPreview.ts) | 不消耗随机数的伤害预览与命中率预览，已计入模组带来的出牌临时加成。 |
| [ops.ts](../../src/engine/ops.ts) | 引擎原语：治疗、护盾、施加状态、属性修正、死亡标记、胜负判定。所有效果最终都落到这里。 |
| [statuses/](../../src/engine/statuses/index.ts) | 状态定义（`STATUS_DEFS`），按类别拆分：增益、减益、持续伤害、控制、各角色专属、废弃楼层敌人专属。叠层与合并规则在 `stacking.ts`。 |
| [pollution.ts](../../src/engine/pollution.ts) / [quirks.ts](../../src/engine/quirks.ts) | 污染累积与抽到污染卡的登记；怪癖（如“生病”）的定义。 |
| [squadBuff.ts](../../src/engine/squadBuff.ts) | 小队增益，以及炼金术士组装部件的收集和兑现。 |
