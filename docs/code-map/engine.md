...
| [types.ts](../../src/engine/types.ts) | 引擎与 UI 共用的类型总集：卡牌、我方/敌方单位、效果、状态、战斗状态、挑战运行态、`EngineOps`、`EncounterModifier`、16 项 `StatBlock`、`StatModifier` 和 `ResistMode`。卡牌支持应星/临时牌、虚无、纳刀、回手费用层与模组来源词条；效果支持手牌选择、预选弃牌、牌面转换、复制入手、逐段随机目标和加算增伤池。 |
...
| [ops.ts](../../src/engine/ops.ts) | 伤害、治疗、护盾、施加状态、战斗内属性修正、弃牌回调和胜负判定等原语；状态实例支持持续拍数、结构化数据、来源记录与清理，并提供丢牌/出牌钩子及护盾击破钩子；伤害修正完成后先结算加算增伤池，再进入命中、暴击、防御、格挡、护盾与 HP 管线。 |
...
| [effects.ts](../../src/engine/effects.ts) | 将 `EffectDescriptor` 解释成引擎原语，并支持保险兑现、有限层数消耗、资源结算向下取整、保险计数伤害加成上限、急诊目标条件；另有 LOSE_HP(按当前生命百分比失去生命)、击杀触发 `onKill`、低血加倍 `damageBonus.targetHpBelowPct`、按本卡累计层数加倍 `bonusMultiplierPerSelfStack`、护盾/治疗的计数加算倍率、组装/移除组装/共鸣、`markPick: "eventCard"`(被动事件卡)与 `convertPick: "handAllFast"`(全速攻转普通并记 `lastConvertBatch`)；并解析 primary、self、allFoes、randomFoe、lowestHpAlly 等目标；随机目标支持按状态过滤，支持护盾回收、按目标有盾/无盾加伤、瀑布/高费用手牌条件、按随机存活角色归属的临时卡入手、培育初始化与递减、体力极限恢复、出牌期临时面板 `PLAY_STAT_BONUS`（目标恒为施放者，写进 `mods` 后记入 `state.playStatMods`，由 `battle.playCard` 在出牌结束逆向撤回）、持续状态、按施放者属性缩放状态层数、星契随机手牌筛选、计数取段数/层数/资源/抽牌、随机回收、弃牌批次速攻统计、全手牌标记和普通手牌转速攻，并提供本次出牌的星辉消耗/瞄准数值加成；`conditionMet` 可额外传入卡牌与目标列表判定条件。卡牌和敌人招式共用；`amount` 固定伤害，`multiplier` 按施放者攻击力计算，二者只能选一个。 |
| [effectsDamage.ts](../../src/engine/effectsDamage.ts) | DAMAGE 的多段伤害管线：支持 onHit、onKill、逐段随机目标、计数增段、目标状态增伤与吸血。 |
| [effectsHand.ts](../../src/engine/effectsHand.ts) | 手牌与牌堆效果：弃牌预选、弃牌堆回收及标记、全手牌标记、牌型转换、培育/共鸣、随机变牌、复制入手和赤潮/纳刀手牌选择。 |
...
| [statuses/](../../src/engine/statuses/) | 状态定义分表：`dot.ts` 负责持续伤害/治疗与反伤, `buffs.ts` 负责增益与锋利增伤池, `swordsman.ts` 负责镜月、铁衣、风切、残心、残心·凝神与八千代, 其余文件负责保险、减益、控制与叠加策略；`index.ts` 合并并提供注册表。 |
