...
| [characters.ts](../../src/data/characters.ts) | 角色颜色、固定 `StatBlock`、统一基础初始卡组和按稀有度分档的个人抽卡池；剑士池已包含全部专属卡，后两名角色仍为占位内容。 |
| [enemies.ts](../../src/data/enemies.ts) | 敌人属性、招式及各自延迟、随机抽招所需数据、每回合行动次数上限、击杀经验和掉落表。经验写在敌人定义中，不写入掉落表。 |
