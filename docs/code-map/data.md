...
| 文件 | 作用 |
| --- | --- |
| [cards.ts](../../src/data/cards.ts) | `CardDef[]`：归属角色、费用、普通/速攻、目标、声明式效果和稀有度；支持本回合弃牌减费、速攻计数联动、弃牌触发、弃牌堆回收、随机手牌标记、应星/瀑布和普通牌转速攻；登记预言家首批 15 张普通、1 张罕见、1 张稀有卡及只由双子星生成的临时《伴星》；攻击牌优先使用攻击力倍率，`text` 支持 `{0}` 等效果数值占位符。 |
