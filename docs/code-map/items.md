...
| 文件 | 作用 |
| --- | --- |
| [types.ts](../../src/items/types.ts) | `ItemDef`、`ItemStack`、类别、装备槽、消耗品用途、掉落条目，以及五档 `ItemRarity`、排序和标签。类别中的 `material` 是制造模组的原料，`module` 是装配到卡牌上的成品模组；两者用途不同。`regionId` 可选标记产出物所属地区，缺省由地区材料规则回落到首个地区。`undroppable` 标记远征中不可通过常规丢弃、替换或放弃拾取移除的物品；`ItemStack.disposable` 只标记实例，表示远征结束销毁、不入仓库且不能退回或寄件；同一 `ItemDef` 可同时存在普通来源与一次性来源。`ItemUse` 不复用 `ExploreEffect`，避免反向依赖探索层。`sellValue`（回收台卖价，只有废料填）与 `buyValue`（据点商店买价，只有装备与材料填）是两个方向、两套数，不要互相换算。 |
| [inventory.ts](../../src/items/inventory.ts) | 背包/仓库共用的容器纯函数。容量由调用方传入，仓库可不传容量；含堆叠占格、溢出原样返回、增删查找、排序和 8×4 视觉排布。并堆与展示合并同时比较 `disposable`，避免一次性物品污染普通物资；`canShipHome` 统一判定物品能否通过投递口寄回。跨 2 格装备不能跨行，`covered` 格渲染时必须跳过；排布不能因 gap 截断物品。 |
| [equipRoll.ts](../../src/items/equipRoll.ts) | 纯装备模型逻辑：模型点换算表、模板校验、主副词条加权块状分配、极端装备代价返还、升阶追加分配、通关奖励完美度增加 `bumpPerfectness`、保留负面代价与原属性方向的完美度重置 `resetPerfectness` 和模型点到实际属性的转换。随机源由调用方注入。 |
