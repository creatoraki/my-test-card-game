...
| 文件 | 作用 |
| --- | --- |
| [types.ts](../../src/items/types.ts) | `ItemDef`、`ItemStack`、类别、装备槽、消耗品用途、掉落条目，以及五档 `ItemRarity`、排序和标签。`ItemUse` 不复用 `ExploreEffect`，避免反向依赖探索层。`sellValue`（回收台卖价，只有废料填）与 `buyValue`（据点商店买价，只有装备与材料填）是两个方向、两套数，不要互相换算。 |
