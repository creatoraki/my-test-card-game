...
| 文件 | 功能 |
| --- | --- |
| `types.ts` | 引擎与 UI 共享的**类型总集**：`Card` / `Combatant`（`Ally`/`Enemy`）/ `EffectDescriptor` / `StatusDef` / `BattleState` / `EngineOps` / **`EncounterModifier`**，以及 **★`StatBlock`（16 项属性面板）与 `StatModifier`（flat/pct 修正层）**、`ResistMode`。概率与百分比一律存**百分点整数**（爆伤 150 = 1.5 倍）。只定义类型、不含逻辑、不 import 其他模块。 |
...
| `stats.ts` | **★ 属性结算的唯一入口**（只 import 类型与规则，谁都能安全引用）：`makeStats`/`addStats`/`applyModifier` 合并面板，`statOf` 读取「局外面板 + 战斗内修正」并对概率类封顶，`addMod` 写入战斗内修正；派生量 `hitChance`（命中率公式）、`defenseMultiplier`、`partyInitiative` 与 `enemyActDelay`（先手排程）、`partyHandLimit`/`partyDrawCount`（小队资源求和）、`burdenPenalty`（负重，背包属 P1 故当前恒 0）。 |
...
| `enemies.ts` | 敌人定义（当前有「怪异的鸟」「废品机器人」「电线杆机器人」「收音机机器人」，占位；后三者技能一致）：`castTick`（**技能基础延迟**，实际间隔还要叠先手差）、`stats`（面板，`attack` 是倍率伤害的基数，首版最弱敌人基础伤害 12~15）、`moves`（招式，复用效果系统）、`script`（循环意图脚本）。 |
...
| `townStore.ts` | **★ 城镇档案**：跨远征持久的养成资产。★ **无等级、无属性点** —— 每个角色一份 `CharacterState`（可用经验池 `exp` / 累计经验 `expEarned` / 个人卡组 `deck` / **卡组等级 `deckLevel`** / **最小卡组下限 `minDeckSize`** / **装备修正层 `equipment`（占位，装备系统未实现）** / 抽卡候选 `pendingDraw`）+ **已唤醒名单 `awakened`** + 上阵名单 `party`（1~3 人，必须 ⊆ `awakened`）+ **居民积分余额 `loot`**。actions：`ensureProfile`（幂等建档）、`resetProfile`、`toggleParty`（未唤醒的人上不了阵）、**`awaken`**（冬眠仓解封：扣 `awakenCost` 积分 + 进 `awakened`，**不自动上阵**）、`grantExp`（★ 经验只进池子，没有升级）、**`bankLoot`**（远征结束落袋），以及**卡组锻造四件套** `upgradeDeck` / `forgeDraw`（摇稀有度 → 3 选 1，候选持久化 → 刷新逃不掉）/ `pickDraw` / `removeCard`（受 `minDeckSize` 拦）/ `lowerMinDeck`（只开放删卡空间，不删卡）。另导出 `deriveStats`（**局外面板唯一换算点**：`(角色基础 + 装备固定) × (1 + 装备百分比)`）与 `countByRarity`/`canAddRarity`（稀有度限携）。**已接 zustand persist（localStorage，key `town-profile-v3`）**。⚠ `characters` 是**全量**建档（未唤醒的人也有一份初始档案，解封即可用），「解锁没解锁」的唯一真相是 `awakened` —— 遍历角色名册时别直接用 `data.CHARACTERS`。 |
| `runStore.ts` | 一次「远征」的**流程编排**：界面路由 + 牌局与战斗之间的往返。只有它同时认识 `battleStore`、`exploreStore` 与 `Screen`，故这三者的连接点全部集中在此。`startExpedition` 建牌局；`enterEncounter` 在打出路线牌后 `launchBattle`——合并上阵角色个人卡组（`structuredClone` 副本）、用 `deriveStats` 生成**完整 `StatBlock` 面板**、**传入继承的 `startHp`**、把危险度经 `encounterModifier` 注入战斗；`resolveBattle` 把战斗终局血量回填给牌局、按危险度倍率发经验（**委托 `townStore.grantExp`**，带幂等护栏）；`confirmExpReport` 回牌桌或进最终结算；`retreat` 落袋回城。⚠ **只有存活角色参战**——本次远征内阵亡的角色不出战，其个人卡组也一并排除出战斗卡组。 |
...
| `BattleScreen.tsx` | 战斗主界面：顶边一条**顶端信息条**（`.battle-topbar`：法力水晶 \| 换牌/丢弃按钮 \| 手牌张数读数 \| 时刻标尺），上半是战场（敌人 + 提示条），下半是一条贯穿全宽的**底部 HUD**（`.battle-hud`：队伍卡 \| 手牌托盘 \| 卡牌说明面板），另有目标选择交互、结束回合按钮、胜负遮罩。★ 仇恨系统移除后，敌人随机选目标，因此**不存在「预计会打谁」的高亮**（原来的仇恨高亮与它的洞察开关一并删掉了）；手牌张数读数的分母改读 `partyHandLimit(battle)`。另含**分镜编排**（`runSteps` 定时器队列）与**场景相机**（`computeCamera`，见下方「分镜相机」）。背景层按当前 `runStore.mapId` 查 `battleBg.ts` 取素材，视频渲染 `<video>`、静态图渲染 `<img>`——两者都只是场景里的一张地皮，自身不带 transform。命中时刻同时触发**顿帧 + 震屏**（见下方「场景动感」）。 |
...
| `ExpRewardScreen.tsx` | **战后小结**：本场缴获的居民积分 + 每个上阵角色一行的经验入账与结算后的可用经验。★ 角色无等级，故**没有升级徽章也没有「距下一级」的进度条**。确认后回到探索牌桌；BOSS 战则进最终结算。标题栏显示本场吃到的净化粒子档位与倍率。 |
| `EndScreen.tsx` | 远征结算：**通关 / 撤退 / 团灭三种收场共用一页**（按 `runStore.lastResult` 切文案）。主角是**轨迹回顾**（复用 `TrailStrip`）——一趟远征结束时那一排卡就是完整的故事。另显示积分入账/遗失、城镇余额、最终危险度与步数，以及上阵角色的**卡组等级**与个人卡组。 |
