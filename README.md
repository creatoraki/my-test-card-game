...
- **手牌与抽牌**：抽牌走**全队固定基准**（`RULES.hand`）—— 第 1 回合抽 `openingDrawCount` = **5** 张，之后每回合抽 `partyBonusDrawCount` = **2** 张，均以手牌上限截断。角色 `drawCount` 属性默认为 0，只有装备/成长带来的加成才会叠加到这两个基准上。小队手牌上限仍是「上阵角色 `handLimit` 之和 + 全队修正」。
...
- **流程**：主菜单 →「开始游戏」→ **据点**（常驻中枢）→ **控制终端 → 下降舱 → 选择地图** → **6 轮区域推进**（每轮：读 20 张节点卡 / 一次性揭示桥接 / 选入口通道 / 逐段推进并在每个节点决定继续或止损 / 本轮线路披露 → 老虎机战斗签 / 三选一 / 推进战斗 / 战后整备）→ 击杀 BOSS 或撤退 → 结算回据点。**当前实现里战斗签那一步是「披露页按下『进入推进战斗』直接开打」。**
  > ⚠ **据点页正在改造中**：现已换成「大厅场景 + 冬眠仓 / 训练室 / 控制终端 / 物资中转仓四个毛玻璃入口」，点击这些入口会播一段**进设施运镜**（镜头以「拿起 → 对焦 → 放大」的节奏推进，前两处短停；界面元素逐个飞出，背景交叉淡入设施场景）。**目前控制终端、冬眠仓与物资中转仓有真实内容**：`ui/ControlTerminalScene.tsx`（下降舱 → 场景内浮层选地图 → 出击；委托 → 三条示例工单的灰化占位，工单系统未接入）、`ui/CryoScene.tsx`（**只剩「冬眠唤醒」一个浮层**）与 `ui/StorageScene.tsx`（库存清单 / 装备调配 / 回收台 三个浮层）；**训练室**进去仍只有背景与「返回据点」。**顶部导航条与「回主菜单」的路仍未恢复**（角落只剩「重置存档」），但**据点已不再是死路** —— 下降舱接的就是 `runStore.startExpedition`，编队是大厅 bento 上的一级入口。⚠ 旧的 `ExpeditionScreen`（远征选图页）已随探索重做一并删除。⚠ **编队与队员档案已从冬眠仓浮层提为据点的一级全屏页**：bento 的「编队」砖（`Facility.kind === "screen"`，唯一一块不播进设施运镜的砖）→ `ui/FormationScreen.tsx`（`screen === "formation"`），点卡面再进 `ui/CharacterDetailScreen.tsx`（`screen === "charDetail"`）；两页与 explore / battle 平级，切换走 ScreenTransition 的默认淡入淡出。冬眠仓由此退回单一职责：只管把人弄醒。
...
├─ 游戏设定.md             # ★ 世界观与机制设定：3300 年代苏醒、电子生命城市、粒子污染探索、经验锻造卡组、关键词模组、据点设施、根性与克隆室、地下城场地
...
├─ 事件设计.md             # ★ 节点事件与战斗签内容设计：事件职责与形态、6 类物品及对应事件类型、部署装置与隐藏区域、轮次与粒子口径、先手/T_attack、污染卡牌（待改为加权）、场景特殊卡牌、体力极限与结疤（未实现）、废弃楼层战斗签模型（8 符号老虎机）
├─ 探索机制评审.md         # 探索机制的玩家体验评审 + **决策记录**：9 条问题的处理结论、原 §2.1 方案的双射漏洞为何被推翻、讨论中新增的 10 项决策及其理由（已写回设计文档；仅「污染加权方向」待拍板）
├─ 角色养成设计.md         # ★ 角色养成设计：无等级的固定基础面板，装备/卡牌/关键词模组养成，根性与永久死亡克隆、属性计算、负重适应和小队资源规则
...
└─ src/
   ...
   │  ├─ types.ts          # 探索层类型总集（路由图 RouteBoard/RouteSegment/RouteBridge、节点事件 NodeEvent（depth 深度分层）、队伍快照、净化粒子档位、推进战斗档位、14 个阶段的会话状态（含「前往下一区域」按下后的**离场行走演出** `leaving`）；★ 背包 backpack·shipped·pendingPickup 与 GAIN_ITEM/ROLL_DROP/DISCARD_SLOTS/OPEN_CHUTE 四种效果在这里）
   ...
   │  ├─ cards.ts          # 卡牌定义（声明式效果）：剑士初始卡 + 按稀有度分档的专属抽卡池、大胃王的占位「基础攻击」、植物学家的占位「普通攻击」；攻击牌写攻击力倍率不写死点数
   │  ├─ characters.ts     # 角色定义（★ 固定基础 StatBlock / 初始卡 / 按稀有度分档的专属抽卡池 pools）；现有剑士、大胃王与植物学家，三人**开局即唤醒并上阵**（名单在 `townStore.INITIAL_AWAKENED`，正好占满 partySize=3）；⚠ 大胃王与植物学家均为占位：面板照抄剑士、初始卡组 5 张基础/普通攻击、抽卡池三档全空
   ...
   │  ├─ exploreStore.ts   # ★ 探索会话状态（包裹 explore/session，克隆式不可变更新）：generateDone / beginReveal / revealDone / pickEntry / arrive / pickOption / confirmNode / pushOn / leaveRegion / leaveDone / startBattle / settleBattle；含背包 action：discardItem / useItem / takePending / abandonPending / shipHome
   ...
   └─ ui/                  # React 视图层（纯展示 + 派发，不含规则）
      ...
      ├─ TownScreen.tsx    # 据点大厅：1920×1080 设计画布（复用 stage.ts，恒 16:9、四周黑边）+ 大厅.png 固定背景 + 右下 718×350 的 bento 毛玻璃面板（**7 块**形状各异的半透明玻璃砖拼成规则矩形：冬眠仓/控制终端/物资中转仓 3 个真设施入口 + **编队**（`kind: "screen"`，★ 唯一一块不是设施的砖：**不播运镜**，直接 `runStore.openFormation` 切到全屏编队页）+ 3 个未开放占位，图标为内联线框 SVG）+ 角落的重置存档；★ 点真入口播「进设施」演出（3s「拿起→对焦→放大」运镜，前两处短停 + 界面元素逐个错峰飞出 + 背景交叉淡入设施场景，参数见 facilityScenes.ts）；★ 设施内容走模块级 FACILITY_CONTENT 登记表（目前 worklog → ControlTerminalScene、cryo → CryoScene、storage → StorageScene）；⚠ 训练室仍是空场景。⚠ 解锁一个设施要**同时**去掉 `locked` 并在 facilityScenes 里登记场景 —— `openable = !locked && hasFacilityScene(id)`
      ...
      ├─ CryoScene.tsx     # ★ 冬眠仓（设施 cryo）的设施内 UI：与 ControlTerminalScene **完全同构**（右侧的**抽屉式入口** + **无遮罩吊绳浮层**，场景上只有标题 + 读数 + 抽屉，点入口才弹浮层），只把亮玻璃的强调色从深青换成**深紫罗兰 #7c4dbe**（冬眠仓.png 是紫粉白的浅色场景，--cryo-* 一套）。⚠ **现在只剩一条抽屉 / 一个浮层**：原「编队」「队员档案」两块已提为一级全屏页（见下面 FormationScreen / CharacterDetailScreen），信息量早撑破 1100×640 的浮层；结构上仍保留「入口砖 → 浮层」这一层，免得加第二个功能时重做骨架。唯一的浮层「冬眠唤醒」= **2 列舱位网格**（已解封 / 密封 / 无信号三态靠灯带·状态灯·冷雾浓度区分，至少 `POD_SLOTS`=6 格）+ 右侧舱位详情（密封舱带缓慢扫描环 + 四格滚动读数）+「解封唤醒 −awakenCost 居民积分」→ `townStore.awaken`。★ 浮层背板上还有一层常驻氛围 `CryoAmbience`（冷凝雾 / 扫描线 / 8 粒霜花，纯 CSS、只动 transform），内容一律在面板落定（`CONTENT_DELAY_MS`）后按 `--i` 错峰浮现。⚠ 卡组锻造（升级/抽卡/删卡/降下限）刻意**不**在这里（按 `游戏设定.md` 的设施分工归训练室）；角色不设等级，也就没有属性加点这回事。⚠⚠ `.cryo-scene`/`.cryo-entries`/`.cryo-modal` 三层永远不能挂 animation/opacity/transform/filter（会成 backdrop root 把玻璃糊死），动画一律挂叶子
      ├─ FormationScreen.tsx # ★ 编队页（据点的**一级全屏页**，`screen === "formation"`，入口＝大厅 bento 的「编队」砖；**不是设施内浮层**，与 explore/battle 平级，切换走 ScreenTransition 默认淡入淡出）：1920×1080 设计画布 + 冬眠仓.png 背景 + **提亮**层（浅色场景压暗会毁气质）+ 同族亮玻璃配方（深紫墨字 / 强调深紫罗兰 #7c4dbe，--fm-* 一套）。★ 版面 = **一整片角色卡阵列**（不是「上阵槽 + 待命区」两段式）：卡片 **260×650 的高楼型（≈1:2.5）**，auto-fill 下 1776px 宽排 6 列，阵列自己滚；是否出战靠**异色边框 + 右上角上阵/下阵角标 + 左上「出战中」标**表达，人数上限靠右上「上阵 n / N」读数与满员置灰的角标说清楚（禁用口径与 townStore.toggleParty 一一对应）。★ 卡面立绘走**半身取景**：图宽 = 卡宽 × `--fm-bust-zoom`(1.5) 顶对齐、腰以下由取景窗裁掉（与 AllyBar 同做法，共用 CharacterPortrait 的 --portrait-dx/dy）。★ 悬浮反馈**只高亮边框**，不动背景也不抬起（整片浅玻璃变底色会很吵）—— ⚠ 为此必须用 `.fm-stage .fm-card-main:hover` (0,3,0) 按住全局 `button:hover:not(:disabled)` (0,2,1) 的暗底配方，否则鼠标一进卡面文字/底色就被刷成 HUD 暗色。⚠ 卡外壳是 div 而非 button（卡面与角标是两个**同级** button，button 套 button 是非法 HTML）
      ├─ CharacterDetailScreen.tsx # ★ 角色详情页（一级全屏页，`screen === "charDetail"`，从编队页点卡面进、返回编队页；原是冬眠仓「队员档案」浮层的右栏）：与编队页同族视觉（同背景 / 同亮玻璃配方，--cd-* 一套），立绘 / 16 项属性 / 个人卡组各占一栏不再互挤。内容**只读** —— 加点与卡组锻造归训练室，角色也不设等级（`角色养成设计.md`），故这里给的是可用经验与卡组锻造进度；唯一改状态的是左下角的**上阵 / 下阵**（禁用口径与编队页、townStore 三处一致）
      ...
      ├─ ExploreScreen.tsx # ★ 探索主界面：1920×1080 设计画布（复用 stage.ts）+ 废弃楼层背景全屏铺满 + 中央**等距区域路由图**面板；四角 HUD —— 左上区域/轮次 n/6/推进段 m/4/阶段提示/本轮战斗档位、右上读数（净化粒子表 / 污染层数 / 居民积分 / ★ 负重实时读数）、**左上空三角里的节点详情侧栏**（等距棋盘沿左下→右上铺开，包围盒的左上/右下两角本来就空着）（悬停任一节点图标即展开完整文本与两条分支预览）、左下队伍血条（复用 HpBar）、右下探索指令（置灰占位，含侧向跨接剩余次数）+ ★ 背包按钮（按 `session.canOpenBackpack` 锁阶段，开 BackpackPanel；背包装不下东西时强制打开且不许推进）+ 撤离；★ 浮层一律落在图的**下方**且无全屏遮罩：landed 选分支 → resolving 看结算 → **atNode 两个并排等重按钮（继续推进：下一段 −3 粒子 · 该段 N 根桥接 ／ 前往下一区域：放弃剩余 K 个节点）** → **leaving 离场行走演出（整块画布锁交互，浮层收起，棋子在图上走）** → **routeDisclosure 披露页 +「进入推进战斗」**；聚焦靠四角 HUD 的 `.is-recede`（只压叶子元素）与落点升起的光柱 `.expl-beam`；⚠ 舞台与玻璃砖的祖先链上禁挂 animation/opacity/transform/filter
      ├─ RouteBoard.tsx    # ★ 等距区域路由图本体（一张 SVG 打全部阶段，**经典战棋的等距平行投影**：2:1 斜率，推进轴 = 左下→右上、通道轴 = 左上→右下，四个方向分别指向左上/右上/左下/右下，整张图是一块斜置的棋盘）：generating **新区域 2 秒沿推进方向被「扫描」出来**（★ 全程只有一个**平行于通道轴、沿推进轴推进的波面**，四件事全按它错开：起点地板沿来路升起落位 → 连线**逐节 dash 绘制**、绘制头跟一点亮火花 → 节点地板从上方砸落并一次性落地辉光 → 一条平行于通道轴的**斜向扫描光带**扫过全图一次；时长由导出的 `GENERATE_MS` 统一，全程锁交互，桥接不出现）→ sealed 图已就位但**桥接仍遮蔽**、正中悬「探索路线」按钮 → revealing **全图 4 段桥接一次性全显** + 顶部倒计时条（时长取按轮次分档的 `revealDurationMs`，★ 一轮仅此一次）→ choosingEntry 桥接**只改 opacity 不卸载**、入口通道 A-E 可点（blockedLanes 置灰）→ advancing 信号顺主管推进、到岔口拐上桥管落到隔壁通道（dash 点亮 + 亮头 + **渐隐拖尾** + 过桥扩散环 + animateMotion 信号点，0.42 设计 px/ms 的慢速；★ **信号点跑完整条折线后，代表玩家的几何体小人才起步，以 1/3 的速度走完同一条线**，一段推进的总时长 = travelMs × 4，落点结算按小人落位时刻排）→ landed/atNode 落点地板被「撞」一下并高亮、已走过的路径保持点亮 → **leaving 离场行走**（★ 选了「前往下一区域」**不再瞬移进战斗**：棋子从当前落点沿本轮**剩余的整条线路**一路走到第 4 段终点，走线 `.rb-trace-walk` 跟着他同速描出、桥接一并显形，走完才进披露页；0.28 设计 px/ms、900–3600ms 夹逼，reduced-motion 压到 420ms；0 节点直推与已走满 4 段没有剩余线路，`session.leaveRegion` 会跳过这一相）→ routeDisclosure 全图桥接常亮 + 实际路径整条描出 + 被放弃的节点压暗；★ 画面语言 = **瓦片地板 + 黑色连线**：起点与 20 个节点都是一块带厚度的等距瓦片地板（2:1 菱形顶面 + 侧面 + 地砖缝，★ **材质跟随事件类型**：顶面/侧面/砖缝按该节点的 `--k` 轻度染色，风险标记 r-negative/r-highRisk 再叠一档、当前落点染得更实，封锁入口压回中性），地板上**站着**一个立体化的 48×48 内联线框 SVG 图标（暗部厚度 / 主体线框 / 受光边三层叠加 + 脚下椭圆投影，不用 emoji），地板之间用**一条嵌进地面的细线**相连（★ 两层同几何描边：近黑轨槽 `.rb-pipe-bed`(5.4) + 细亮线 `.rb-pipe`(2.4)，圆头端帽插进地板，不是圆柱水管；亮线读作躺在凹槽里的导体，通道整组另挂一层极淡辉光；端点/端帽照着电流特效定，电流(4)比亮线粗一档、跑过去把它整条盖住只留两侧一线暗边）；⚠ **桥线与主线平时完全同款**（★ 唯一例外：**揭示期的桥接换成闪烁的亮青 + 加粗 + 辉光**，与细白底线拉开颜色对比 —— 这是全页禁闪烁约定的唯一例外，只限 `.rb-bridges.is-revealing`，披露页的桥接仍是常亮不闪），等距下「两轴撞车」的风险改由三件事承担——① 朝向天然相反（主线右上 / 桥线右下）② 地板只长在推进轴上 ③ 桥线平时根本不在画面上；节点文字详情走 ExploreScreen 的侧栏；入口 A-E 是**同款地板 + 立体字母**（多层 text-shadow 挤出厚度，常态极缓慢上下浮动、悬停定住抬起并把所属通道整串连线通电，封锁态地板描边转虚线、字母划掉）；★ **玩家棋子层**（`<Pawn>`：脚底为原点的实心等距几何体小人 = 棱台躯干 + 立方体头，顶面亮/右侧面中/左侧面暗三档 + 荧光描边 + 脚下椭圆投影，与线框事件图标一实一虚区分「我」和「事件」；单独一层 z-index:3 压在地板之上、整层套 TILE_NUDGE 与地板对齐；选完入口后常驻当前地板并缓慢呼吸，推进时由 animateMotion 沿本段折线行走并带踩踏起伏；⚠ 起步等待用 keyPoints 而非 begin 延迟、且必须 `begin="indefinite"` + `beginElement()` 手动启动 —— SMIL 的 begin=0 相对时间容器算，第二段之后会直接跳到终态）；⛔ 全页禁止闪烁型明暗变化
      ...
      ├─ BattleScreen.tsx  # ★ 战斗主界面：顶端信息条（贴死画布上沿、无切角零缝隙，高度走 --topbar-h：法力/换牌丢弃/手牌读数/时刻标尺/★ 青蓝科技键「结束回合」）+ 敌我单位/手牌/胜负遮罩 + 分镜编排 + 场景相机（世界坐标）。★ 底部 HUD 为**两列**（队伍卡 500 | 手牌托盘铺到画布右缘），卡牌说明面板已独立到画布右上角；手牌卡 200×336，叠压量按张数自适应（手牌上限实为 partyHandLimit() 的 10 张）
      ...
      ├─ HandCard.tsx      # 手牌单卡（200×336 三段式卡面：顶行 52 高，3D 水晶费用 + 卡名 / 中间 200 见方 1:1 配图 / 底部 84 高效果说明；发牌飞入 / 出鞘离场 / 机能边框 + 巡游流光；悬停只向上弹出半张卡高、不放大）
      ├─ CardView.tsx      # 单张卡牌（奖励/结算界面用）
      ├─ CardInfoPanel.tsx # 画布右上角的固定卡牌说明面板（1:2 竖版：上半 1:1 大卡面 + 下半信息；无卡时科幻待机占位；★ 已从底部 HUD 第三列搬出，整列宽度让给手牌托盘）
      ├─ ManaCrystalIcon.tsx # 「光」资源水晶图标（内联 3D SVG 宝石，中央桌面留给费用数字）
      ├─ cardArt.ts        # 卡面配图查找表（⚠ 大胃王的「基础攻击」与植物学家的「普通攻击」暂用 1:1 占位素材）
