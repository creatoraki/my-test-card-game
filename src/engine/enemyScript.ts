import type { BattleState, Enemy, EnemyAiMemory, StatusInstance } from "./types";
import type { EnemyDef, EnemyMove, MoveBiasWhen } from "../data";
import { alliesOf, foesOf, tauntedAmong } from "./targeting";
import { rngFloat, rngPick, rngPickWeighted } from "./rng";
...
function biasConditionMet(state: BattleState, enemy: Enemy, condition: MoveBiasWhen): boolean {
  ...
  switch (condition.when) {
    ...
    case "allyCountAtLeast":
      return alliesOf(state, enemy).length >= condition.value;
  }
}
...
export function pickScriptedTarget(
  ...
  if (tauntedAmong(foesOf(state, enemy)).length > 0) return undefined;
