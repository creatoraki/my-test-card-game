...
/** 探索级遗物只解释资源效果，其余效果交给战斗引擎处理。 */
export function fireExploreRelic(s: ExploreState, event: RelicEvent): void {
  for (const stack of relicsInBackpack(s)) {
    const def = getItemDef(stack.itemId);
    const spec = def.relic;
    if (!spec || spec.scope !== "explore") continue;
    const triggers: readonly string[] = Array.isArray(spec.on) ? spec.on : [spec.on];
