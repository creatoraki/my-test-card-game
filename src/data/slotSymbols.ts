...
//
// ⚠ 降级说明(degraded: true 的三张): 设计文档给「环境压力 / 场景战术 / 战场改造」写的效果是
//   「开战污染卡牌」与「场景特殊卡牌」, 这两样引擎侧都还没有 —— engine/EncounterModifier 目前
//   只认「追加敌人 / 敌方开局状态 / 敌方先手 / 敌人 HP 倍率」四项。与其在卡面上承诺一个不执行的
//   效果, 不如降级成语义等价的可落地项, 并把 desc 写成**真实生效的那一句**。
//   等引擎补上场景卡与污染卡, 回来改这三条 + 去掉 degraded 即可, 别的都不用动。
