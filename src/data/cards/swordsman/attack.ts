// 剑士 · 攻击卡。设计口径见 design/卡牌设计/剑士/剑士新卡.md。
import type { CardDef } from "../../../engine/types";

export const SWORDSMAN_ATTACK_CARDS: CardDef[] = [
  {
    id: "snowflake",
    name: "雪花",
    ownerCharId: "swordsman",
    cost: 1,
    cardType: "normal",
    targeting: "foe",
    rarity: "common",
    anim: "ice",
    effects: [{ type: "DAMAGE", multiplier: 0.8, target: "primary", hitBonus: 10 }],
    onDiscard: { mode: "useSelf", autoTarget: "randomFoe" },
    text: "造成 {0} 点伤害，命中 +10%。被丢弃时，自动对随机敌人使用本卡。",
  },
  {
    id: "fallen-leaf",
    name: "落叶",
    ownerCharId: "swordsman",
    cost: 1,
    cardType: "normal",
    targeting: "foe",
    rarity: "common",
    anim: "slash",
    effects: [
      { type: "DAMAGE", multiplier: 1.0, target: "primary", hitBonus: -10 },
      { type: "DISCARD", amount: 1, discardPick: "handBottom" },
    ],
    text: "造成 {0} 点伤害，命中 -10%，然后丢弃手牌最后一张。",
  },
  {
    id: "kagutsuchi",
    name: "迦具土",
    ownerCharId: "swordsman",
    cost: 4,
    // per: 本回合每丢弃 1 张牌，费用再 -1（线性递减，见 engine/cost.ts）。
    costRule: { when: "discardedThisRound", delta: -1, per: true },
    cardType: "normal",
    targeting: "foe",
    rarity: "uncommon",
    exhaust: true,
    anim: "fire",
    effects: [
      { type: "PLAY_STAT_BONUS", stat: "armorPen", amount: 5 },
      { type: "DAMAGE", multiplier: 1.6, target: "primary" },
    ],
    text: "造成 {1} 点伤害；穿甲 +5。本回合每丢弃 1 张牌，本卡费用 -1。打出后消耗。",
  },
  {
    id: "blood-ruin",
    name: "血坏",
    ownerCharId: "swordsman",
    cost: 2,
    cardType: "normal",
    targeting: "foe",
    rarity: "common",
    anim: "blood-slash",
    effects: [
      { type: "PLAY_STAT_BONUS", stat: "armorPen", amount: 5 },
      { type: "LOSE_HP", pctOfCurrentHp: 0.1, target: "self" },
      { type: "DISCARD", amount: 2, discardPick: "handBottom" },
      {
        type: "DAMAGE",
        multiplier: 0.9,
        bonusMultiplierFrom: "lastDiscardBatch",
        bonusMultiplierPer: 0.35,
        target: "primary",
      },
    ],
    text: "失去自身 10% 当前生命，丢弃手牌最后 2 张，然后造成 {3} 点伤害；每丢弃 1 张，伤害倍率 +35%。穿甲 +5。",
  },
  {
    id: "phantom-moon",
    name: "幻月",
    ownerCharId: "swordsman",
    cost: 2,
    cardType: "normal",
    targeting: "foe",
    rarity: "common",
    anim: "iai-slash",
    effects: [
      { type: "DISCARD", amount: 1, discardPick: "handTop" },
      // 基础倍率 0，全部伤害来自「被丢弃牌的费用 × 60%」。
      {
        type: "DAMAGE",
        multiplier: 0,
        bonusMultiplierFrom: "lastDiscardBatchCost",
        bonusMultiplierPer: 0.6,
        target: "primary",
      },
    ],
    text: "丢弃手牌第一张，造成等同于该牌费用 ×60% 攻击力的伤害。",
  },
  {
    id: "gale",
    name: "岚",
    ownerCharId: "swordsman",
    cost: 2,
    // 被丢弃回手累计满 5 层后费用 -2（见 engine/cost.ts stackCostRule）。
    stackCostRule: { atLeast: 5, delta: -2 },
    cardType: "normal",
    targeting: "allFoes",
    rarity: "common",
    anim: "slash",
    effects: [
      {
        type: "DAMAGE",
        multiplier: 0.65,
        bonusMultiplierPerSelfStack: 0.2,
        target: "allFoes",
        hitBonus: -10,
      },
    ],
    onDiscard: { mode: "returnToHand", maxStacks: 5 },
    text: "对所有敌人造成 {0} 点伤害，命中 -10%。被丢弃时回到手牌，伤害倍率 +20%，最多累计 5 层；满 5 层时费用 -2。打出后累计清零。",
  },
  {
    id: "falling-sakura",
    name: "落樱",
    ownerCharId: "swordsman",
    cost: 2,
    cardType: "fast",
    targeting: "foe",
    rarity: "common",
    anim: "slash",
    effects: [
      { type: "DAMAGE", multiplier: 1.0, target: "primary" },
      {
        type: "DAMAGE",
        multiplier: 0.2,
        hitsFrom: "discardsThisBattle",
        maxHits: 5,
        target: "primary",
      },
    ],
    text: "造成 {0} 点伤害；本场战斗每丢弃过 1 张牌，额外造成 1 段 {1} 点伤害，最多 5 段。",
  },
  {
    id: "spring-sprout",
    name: "春芽",
    ownerCharId: "swordsman",
    cost: 1,
    cardType: "fast",
    targeting: "foe",
    rarity: "uncommon",
    anim: "slash",
    effects: [
      { type: "DAMAGE", multiplier: 0.7, target: "primary", hitBonus: 10 },
      { type: "DRAW", amount: 1, condition: "noPlaysThisRound" },
    ],
    text: "造成 {0} 点伤害，命中 +10%。本回合没有使用过其他牌时，抽 1 张牌。",
  },
  {
    id: "wolf-sparrow",
    name: "狼雀",
    ownerCharId: "swordsman",
    cost: 2,
    cardType: "fast",
    targeting: "foe",
    rarity: "uncommon",
    anim: "slash",
    effects: [
      { type: "PLAY_STAT_BONUS", stat: "critRate", amount: 20 },
      {
        type: "DAMAGE",
        multiplier: 1.1,
        // 目标生命低于 30% 时倍率改为 160%（1.1 + 0.5）。
        damageBonus: { when: "targetHpBelowPct", value: 30, multiplier: 0.5 },
        onKill: [{ type: "DRAW", amount: 2, target: "self" }],
        target: "primary",
      },
    ],
    text: "造成 {1} 点伤害；目标生命低于 30% 时伤害倍率改为 160%；暴击 +20%。击杀时抽 2 张牌。",
  },
  {
    id: "rift-light",
    name: "天隙流光",
    ownerCharId: "swordsman",
    cost: 3,
    cardType: "fast",
    targeting: "foe",
    rarity: "rare",
    exhaust: true,
    anim: "lightning",
    // 先丢弃再结算：伤害倍率按本次丢弃张数加算，一次结算完成（不拆成两段飘字）。
    effects: [
      { type: "PLAY_STAT_BONUS", stat: "critRate", amount: 20 },
      { type: "DISCARD", discardPick: "handAll" },
      {
        type: "DAMAGE",
        multiplier: 1.8,
        bonusMultiplierFrom: "lastDiscardBatch",
        bonusMultiplierPer: 0.45,
        target: "primary",
      },
    ],
    text: "造成 {2} 点伤害，暴击 +20%，然后丢弃所有手牌；每丢弃 1 张，额外造成 45% 攻击力的伤害。打出后消耗。",
  },
  {
    id: "flying-plum",
    name: "飞梅",
    ownerCharId: "swordsman",
    cost: 1,
    cardType: "normal",
    targeting: "foe",
    rarity: "common",
    anim: "ice",
    effects: [
      {
        type: "DAMAGE",
        multiplier: 0.7,
        target: "primary",
        hitBonus: 15,
        onHit: [{ type: "TRANSFORM_CARD", cardId: "snowflake", amount: 1 }],
      },
    ],
    text: "造成 {0} 点伤害，命中 +15%；命中后随机将 1 张手牌变为雪花。",
  },
  {
    id: "windcut",
    name: "风切",
    ownerCharId: "swordsman",
    cost: 1,
    cardType: "fast",
    targeting: "foe",
    rarity: "common",
    anim: "blade-slash",
    effects: [
      { type: "DAMAGE", multiplier: 0.6, target: "primary" },
      { type: "APPLY_STATUS", status: "windCut", stacks: 1, duration: 2, target: "self" },
    ],
    text: "造成 {0} 点伤害，并获得风切：命中率 +10%，持续 2 回合。",
  },
  {
    id: "thunder-run",
    name: "雷走",
    ownerCharId: "swordsman",
    cost: 1,
    cardType: "fast",
    targeting: "foe",
    rarity: "common",
    anim: "lightning",
    playReturn: { when: "fastPlaysThisRound", atLeast: 1, costDelta: 1 },
    effects: [{ type: "DAMAGE", multiplier: 0.7, target: "primary", hitBonus: 10 }],
    text: "造成 {0} 点伤害，命中 +10%。本回合已打出其他速攻牌时，打出后回到手牌且费用 +1。",
  },
  {
    id: "shigure",
    name: "时雨",
    ownerCharId: "swordsman",
    cost: 2,
    cardType: "fast",
    targeting: "none",
    rarity: "uncommon",
    anim: "triple-strike",
    effects: [
      {
        type: "DAMAGE",
        multiplier: 0.4,
        hits: 3,
        target: "randomFoe",
        randomPerHit: true,
        bonusHitsFrom: "fastPlaysThisRound",
        maxBonusHits: 3,
      },
    ],
    text: "对随机敌人造成 3 段 {0} 点伤害；本回合每打出 1 张速攻牌，额外增加 1 段，最多 3 段。",
  },
];
