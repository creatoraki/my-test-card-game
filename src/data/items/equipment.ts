import type { ItemDef } from "../../items/types";

const DEFS: ItemDef[] = [
  {
    id: "dawn-saber",
    name: "曙光军刀",
    category: "equipment",
    rarity: "common",
    desc: "普通级武器模板：提高攻击力与命中率。",
    maxStack: 1,
    slot: "weapon",
    familyId: "dawn-saber",
    mods: { flat: { attack: 2 }, pct: { hitRate: 6 } },
    icon: "weapon",
  },
  {
    id: "armor-piercing-crossbow",
    name: "穿甲弩",
    category: "equipment",
    rarity: "common",
    desc: "普通级武器模板：提高攻击力与精准。",
    maxStack: 1,
    slot: "weapon",
    familyId: "armor-piercing-crossbow",
    mods: { flat: { attack: 2 }, pct: { precision: 6 } },
    icon: "weapon",
  },
  {
    id: "hunting-rifle",
    name: "猎杀步枪",
    category: "equipment",
    rarity: "common",
    desc: "普通级武器模板：提高暴击率与爆伤。",
    maxStack: 1,
    slot: "weapon",
    familyId: "hunting-rifle",
    mods: { pct: { critRate: 12, critDamage: 30 } },
    icon: "weapon",
  },
  {
    id: "quickstrike-gauntlet",
    name: "迅击拳套",
    category: "equipment",
    rarity: "common",
    desc: "普通级武器模板：提高攻击力与先手。",
    maxStack: 1,
    slot: "weapon",
    familyId: "quickstrike-gauntlet",
    mods: { flat: { attack: 2, initiative: 3 } },
    icon: "weapon",
  },
  {
    id: "shock-maul",
    name: "震荡重锤",
    category: "equipment",
    rarity: "common",
    desc: "普通级武器模板：提高攻击力与爆伤。",
    maxStack: 1,
    slot: "weapon",
    familyId: "shock-maul",
    mods: { flat: { attack: 1 }, pct: { critDamage: 20 } },
    icon: "weapon",
  },
  {
    id: "cross-sword",
    name: "十字剑",
    category: "equipment",
    rarity: "common",
    desc: "普通级极端武器模板：提高最大生命值和攻击力，但降低先手。",
    maxStack: 1,
    slot: "weapon",
    familyId: "cross-sword",
    mods: { flat: { maxHp: 10, attack: 3, initiative: -3 } },
    icon: "weapon",
  },
  {
    id: "glass-dagger",
    name: "玻璃匕首",
    category: "equipment",
    rarity: "common",
    desc: "普通级极端武器模板：提高暴击率和爆伤，但降低最大生命值。",
    maxStack: 1,
    slot: "weapon",
    familyId: "glass-dagger",
    mods: { flat: { maxHp: -20 }, pct: { critRate: 15, critDamage: 30 } },
    icon: "weapon",
  },
  {
    id: "heavy-cannon",
    name: "重型火炮",
    category: "equipment",
    rarity: "common",
    desc: "普通级极端武器模板：提高攻击力和爆伤，但降低命中率与先手。",
    maxStack: 1,
    slot: "weapon",
    familyId: "heavy-cannon",
    mods: { flat: { attack: 3, initiative: -3 }, pct: { critDamage: 30, hitRate: -8 } },
    icon: "weapon",
  },
  {
    id: "hunter-longbow",
    name: "追猎长弓",
    category: "equipment",
    rarity: "common",
    desc: "普通级极端武器模板：提高精准和暴击率，但降低攻击力。",
    maxStack: 1,
    slot: "weapon",
    familyId: "hunter-longbow",
    mods: { flat: { attack: -2 }, pct: { precision: 8, critRate: 10 } },
    icon: "weapon",
  },
  {
    id: "deflection-blade",
    name: "偏折盾刃",
    category: "equipment",
    rarity: "common",
    desc: "普通级极端武器模板：提高闪避率和格挡率，但降低攻击力。",
    maxStack: 1,
    slot: "weapon",
    familyId: "deflection-blade",
    mods: { flat: { attack: -3 }, pct: { dodgeRate: 8, blockRate: 8 } },
    icon: "weapon",
  },
  {
    id: "city-defense-heavy-armor",
    name: "城防重甲",
    category: "equipment",
    rarity: "common",
    desc: "普通级防具模板：提高防御力与最大生命值。",
    maxStack: 1,
    slot: "armor",
    familyId: "city-defense-heavy-armor",
    mods: { flat: { defense: 4, maxHp: 15 } },
    icon: "armor",
  },
  {
    id: "composite-armor",
    name: "复合护甲",
    category: "equipment",
    rarity: "common",
    desc: "普通级防具模板：稳定提高防御力与最大生命值。",
    maxStack: 1,
    slot: "armor",
    familyId: "composite-armor",
    mods: { flat: { defense: 3, maxHp: 20 } },
    icon: "armor",
  },
  {
    id: "guard-plate",
    name: "护卫板甲",
    category: "equipment",
    rarity: "common",
    desc: "普通级防具模板：提高防御力与格挡率。",
    maxStack: 1,
    slot: "armor",
    familyId: "guard-plate",
    mods: { flat: { defense: 5 }, pct: { blockRate: 5 } },
    icon: "armor",
  },
  {
    id: "hazmat-suit",
    name: "防毒战衣",
    category: "equipment",
    rarity: "common",
    desc: "普通级防具模板：提高防御力与异常抗性。",
    maxStack: 1,
    slot: "armor",
    familyId: "hazmat-suit",
    mods: { flat: { defense: 2 }, pct: { ailmentResist: 10 } },
    icon: "armor",
  },
  {
    id: "mobile-armor",
    name: "机动护甲",
    category: "equipment",
    rarity: "common",
    desc: "普通级防具模板：提高防御力与闪避率。",
    maxStack: 1,
    slot: "armor",
    familyId: "mobile-armor",
    mods: { flat: { defense: 2 }, pct: { dodgeRate: 5 } },
    icon: "armor",
  },
  {
    id: "buffer-coat",
    name: "缓冲外套",
    category: "equipment",
    rarity: "common",
    desc: "普通级防具模板：提高防御力与护盾强度。",
    maxStack: 1,
    slot: "armor",
    familyId: "buffer-coat",
    mods: { flat: { defense: 3 }, pct: { shieldBoost: 8 } },
    icon: "armor",
  },
  {
    id: "life-support-armor",
    name: "生命维持甲",
    category: "equipment",
    rarity: "common",
    desc: "普通级防具模板：提高防御力与最大生命值。",
    maxStack: 1,
    slot: "armor",
    familyId: "life-support-armor",
    mods: { flat: { defense: 2, maxHp: 25 } },
    icon: "armor",
  },
  {
    id: "assault-exoskeleton",
    name: "突击外骨骼",
    category: "equipment",
    rarity: "common",
    desc: "普通级防具模板：少量提高攻击力与防御力。",
    maxStack: 1,
    slot: "armor",
    familyId: "assault-exoskeleton",
    mods: { flat: { attack: 2, defense: 1 } },
    icon: "armor",
  },
  {
    id: "fire-control-armor",
    name: "火控战甲",
    category: "equipment",
    rarity: "common",
    desc: "普通级防具模板：提高命中率、攻击力与防御力。",
    maxStack: 1,
    slot: "armor",
    familyId: "fire-control-armor",
    mods: { flat: { attack: 1, defense: 1 }, pct: { hitRate: 5 } },
    icon: "armor",
  },
  {
    id: "raider-light-armor",
    name: "破袭轻甲",
    category: "equipment",
    rarity: "common",
    desc: "普通级防具模板：提高攻击力、暴击率与防御力。",
    maxStack: 1,
    slot: "armor",
    familyId: "raider-light-armor",
    mods: { flat: { attack: 1, defense: 1 }, pct: { critRate: 6 } },
    icon: "armor",
  },
  {
    id: "tactical-goggles",
    name: "战术目镜",
    category: "equipment",
    rarity: "common",
    desc: "普通级饰品模板：提高攻击力、命中率与精准。",
    maxStack: 1,
    slot: "trinket",
    familyId: "tactical-goggles",
    mods: { flat: { attack: 1 }, pct: { hitRate: 5, precision: 5 } },
    icon: "trinket",
  },
  {
    id: "polarized-core",
    name: "偏振核心",
    category: "equipment",
    rarity: "common",
    desc: "普通级饰品模板：提高防御力、攻击力与护盾强度。",
    maxStack: 1,
    slot: "trinket",
    familyId: "polarized-core",
    mods: { flat: { defense: 2, attack: 1 }, pct: { shieldBoost: 6 } },
    icon: "trinket",
  },
  {
    id: "quickstep-watch",
    name: "急行怀表",
    category: "equipment",
    rarity: "common",
    desc: "普通级饰品模板：提高先手、闪避率与暴击率。",
    maxStack: 1,
    slot: "trinket",
    familyId: "quickstep-watch",
    mods: { flat: { initiative: 2 }, pct: { dodgeRate: 5, critRate: 6 } },
    icon: "trinket",
  },
  {
    id: "medical-pendant",
    name: "医疗吊坠",
    category: "equipment",
    rarity: "common",
    desc: "普通级饰品模板：提高治愈力、治愈强度与最大生命值。",
    maxStack: 1,
    slot: "trinket",
    familyId: "medical-pendant",
    mods: { flat: { healPower: 3, maxHp: 8 }, pct: { healBoost: 8 } },
    icon: "trinket",
  },
  {
    id: "resource-chip",
    name: "资源芯片",
    category: "equipment",
    rarity: "common",
    desc: "普通级饰品模板：提高全队手牌、抽牌和负重适应。",
    maxStack: 1,
    slot: "trinket",
    familyId: "resource-chip",
    mods: { flat: { handLimit: 1, drawCount: 1 }, pct: { burdenAdapt: 10 } },
    icon: "trinket",
  },
  {
    id: "reaction-charm",
    name: "反应护符",
    category: "equipment",
    rarity: "common",
    desc: "普通级饰品模板：提高防御力、格挡率与异常抗性。",
    maxStack: 1,
    slot: "trinket",
    familyId: "reaction-charm",
    mods: { flat: { defense: 2 }, pct: { blockRate: 5, ailmentResist: 8 } },
    icon: "trinket",
  },
  {
    id: "critical-prism",
    name: "暴击棱镜",
    category: "equipment",
    rarity: "common",
    desc: "普通级饰品模板：提高暴击率、爆伤与攻击力。",
    maxStack: 1,
    slot: "trinket",
    familyId: "critical-prism",
    mods: { flat: { attack: 1 }, pct: { critRate: 8, critDamage: 20 } },
    icon: "trinket",
  },
  {
    id: "life-thorn-ring",
    name: "生命棘环",
    category: "equipment",
    rarity: "common",
    desc: "普通级饰品模板：提高最大生命值、防御力与治愈力。",
    maxStack: 1,
    slot: "trinket",
    familyId: "life-thorn-ring",
    mods: { flat: { maxHp: 15, defense: 1, healPower: 2 } },
    icon: "trinket",
  },
  {
    id: "breach-beacon",
    name: "破阵信标",
    category: "equipment",
    rarity: "common",
    desc: "普通级饰品模板：提高攻击力、精准与命中率。",
    maxStack: 1,
    slot: "trinket",
    familyId: "breach-beacon",
    mods: { flat: { attack: 2 }, pct: { precision: 6, hitRate: 5 } },
    icon: "trinket",
  },
  {
    id: "burden-module",
    name: "负重模块",
    category: "equipment",
    rarity: "common",
    desc: "普通级饰品模板：提高负重适应、闪避率与防御力。",
    maxStack: 1,
    slot: "trinket",
    familyId: "burden-module",
    mods: { flat: { defense: 1 }, pct: { burdenAdapt: 15, dodgeRate: 4 } },
    icon: "trinket",
  },
];

// 本表 30 件全部掉落时 roll 一条随机羁绊词条(《羁绊设计概览.md》§2.1:
// 普通武器/防具/饰品各提供 1 条随机羁绊; 稀有度**不**增加词条数量)。
// ★ 在这里统一打标而不是逐条写 affinityRollable —— 这张表里每一件都是普通装备,
//   逐条写 30 遍只会让「漏写一条」变成沉默的 bug(那件装备会永远不带词条)。
//   日后若出现不给随机词条的装备(如羁绊饰品), 再改成逐条声明。
export const EQUIPMENT_ITEM_DEFS: ItemDef[] = DEFS.map((d) => ({ ...d, affinityRollable: true }));
