// 数据注册表的 getter 与实例工厂 —— 所有内容按 id 索引。
// ★ 单独成文件是为了打断依赖环: data 下的子模块(卡牌模组、地图补给、每日奖励等)需要这些 getter,
//   直接引本文件即可, 不要再从聚合入口 data/index 取 —— 那会形成「入口 ↔ 子模块」的运行时环。
...
import { DESIGN_ITEM_DEFS, EQUIPMENT_ITEM_DEFS } from "../items";
import { CHARACTERS, type CharacterDef } from "../roster/characters";
