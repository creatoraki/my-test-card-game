...
const HAZARD: NodeEvent[] = [
  ...
  },
  {
    ...
    choices: [
      ...
      choice("disconnect", "断开样品舱", "切断主缆线，可能污染卡牌或拆出魔方", "你拔掉连接样品柜和实验室的主缆线，把异常冻结限制在柜体内部。", [
        ...
        chance("disconnect-d", 20, "异常场震碎了样品柜的识别核心；你们拆出魔方 ×1，但卡牌《{实际卡牌名}》被污染，受影响角色为{实际角色名}。", [contaminate(1), item("logic-cube")]),
      ]),
    ],
  },
  {
  ...
  },
  {
    ...
    energyDelta: 0,
    choices: [
      choice("safe-line", "沿安全标线绕行", "沿标线移动，可能消耗粒子或找到通用材料", "你沿着地面上褪色的白线走，尽量不触碰靶场中央的感应区。", [
        chance("safe-line-a", 55, "褪色的安全标线在队伍脚下重新亮起，你们绕过感应区，靶场没有启动炮台。", []),
        chance("safe-line-b", 25, "标线反复重新校准，队伍额外消耗 5 点粒子等待通道稳定后通过。", [energy(-5)]),
        chance("safe-line-c", 20, "安全标线在尽头指向一处维护槽，你们从里面取出魔方 ×1，然后离开靶场。", [item("logic-cube")]),
      ]),
...

  {
    ...
    energyDelta: 0,
    choices: [
      choice("wait", "等待量子锁自行解密", "等待自检，可能消耗粒子或拆出通用材料", "你保持距离，等待封存柜按照旧协议完成自检。", [
        chance("wait-a", 70, "量子封存柜完成了漫长的自检，却只释放出一段空白索引；队伍额外消耗 6 点粒子后离开档案库。", [energy(-6)]),
        chance("wait-b", 30, "量子锁完成解密，封存柜的识别核心弹出；你们拆出魔方 ×1。", [item("logic-cube")]),
      ]),
      choice("copy", "趁警报间隙复制档案", "截取缓存，可能污染 1-2 张卡牌或拆出通用材料", "你不打开柜门，只在警报短暂熄灭时截取量子缓存。", [
        ...
        chance("copy-c", 25, "缓存弹出一枚仍能工作的逻辑核心；你们带走魔方 ×1，但反向追踪污染了{实际角色名}的卡牌《{实际卡牌名}》。", [contaminate(1), item("logic-cube")]),
      ]),
      choice("break-locker", "破坏封存柜外壳", "从外部切开，可能受伤或拆出通用材料", "你用工具从外部切开柜体，试图避开量子锁。", [
        ...
        chance("break-locker-c", 30, "压缩场擦过队伍，全队损失 6% HP；你们从柜内抢出魔方 ×1。", [dmg(0.06), item("logic-cube")]),
      ]),
      choice("overwrite", "覆写量子密钥", "强行夺权，可能污染、受伤、消耗粒子并拆出通用材料", "你把自己的临时密钥写进封存柜，让系统把队伍当成新的档案管理员。", [
        ...
        chance("overwrite-d", 30, "旧权限的清除脉冲让全队损失 10% HP；你们拆出魔方 ×1，但卡牌《{实际卡牌名}》被污染。受影响角色为{实际角色名}。", [dmg(0.1), contaminate(1), item("logic-cube")]),
      ]),
    ],
  },
  {
  ...
  },
  {
    ...
    energyDelta: 0,
    choices: [
      choice("verify", "接受普通身份验证", "按流程验证，可能消耗粒子或拆出通用材料", "你用最普通的访客身份递交访问请求，等待终端按流程核验。", [
        chance("verify-a", 80, "终端按普通访客流程反复查询旧数据库，队伍额外消耗 4 点粒子后通过验证。", [energy(-4)]),
        chance("verify-b", 20, "普通身份验证意外匹配到一份被遗忘的董事会记录；终端开启设备仓，你们拆出魔方 ×1。", [item("logic-cube")]),
      ]),
