...
- `废弃楼层/默认宝箱素材.png`：256×148 单张交互物图，所有可交互物共用；素材底部 20px 透明留白，
  比例登记在 `CORRIDOR_PROP_GROUND_TRIM`，渲染时通过负外边距补偿。每种物件另有独立的
  `CORRIDOR_PROP_Y_OFFSETS` 场景 Y 轴微调值（设计 px，正值向下）。
