...
- `废弃楼层/近景/测试.png`（2048×771）、`测试2.png`（2172×724）：两种近景图都按 1080px 高铺满每间房 3840×1080 的地图范围；新手关卡在生成房间时随机混排两种地图并保证各至少出现一次，房间重访时保持不变。两张图的平台顶面分别由 `CORRIDOR_LAYOUT.nearMapFloorY` 对齐到 `CORRIDOR.floorY`。
