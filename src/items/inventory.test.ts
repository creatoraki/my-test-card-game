...
describe("占格", () => {
  it("一件物品一格, 装备也不例外", () => {
    expect(occupiedSlots([stack("copper-coin")], getItemDef)).toBe(1);
    expect(occupiedSlots([stack("armor-plate-c")], getItemDef)).toBe(1);
    expect(occupiedSlots([stack("copper-coin"), stack("prybar-c")], getItemDef)).toBe(2);
  });

  it("容量满了就溢出, 不会静默丢弃", () => {
    const full = Array.from({ length: 24 }, () => stack("copper-coin"));
    ...
    const b = addToContainer(full.slice(0, 23), [stack("copper-coin")], getItemDef, 24);
...
describe("背包排布", () => {
  it("至少铺满容量, 空位补 empty", () => {
    const cells = layoutBackpack([stack("copper-coin")], 24);
  ...
  it("装备与其余物品一样只占一格, 顺序与数组一致", () => {
    const stacks = [stack("armor-plate-c"), stack("copper-coin")];
  ...
  // 满包时不截断: 一件东西都不能藏起来。
  it("满包时每件都有格子", () => {
    const stacks = Array.from({ length: 24 }, () => stack("copper-coin"));
...
describe("掉落", () => {
  ...
  it("同种子的掉落逐件复现", () => {
    const table = [
      { kind: "item" as const, itemId: "copper-coin", chance: 0.6, min: 1, max: 2 },
