...
export function StorageScene({ leaving = false }: Props) {
  ...
  return (
    <div className={`stor-scene${leaving ? " is-leaving" : ""}`}>
      {/* ---- 左上: 场景标题 ---- */}
      <header className="stor-header" style={{ left: "56px", top: "42px" }}>
        <span className="stor-kicker">SUPPLY DEPOT</span>
        <h2 className="stor-title">物资中转仓</h2>
        <p className="stor-sub">库存管理 · 装备调配 · 废料回收</p>
      </header>

      {/* ---- 右上: 三枚读数 chip ---- */}
      <div className="stor-readout" style={{ right: "56px", top: "42px" }}>
        <div className="stor-chip">
          <span className="stor-chip-label">库存</span>
          {/* 仓库不设上限, 所以只报件数与占格, 不报「x / 上限」 */}
          <strong className="stor-chip-value">{storage.length}</strong>
          <span className="stor-chip-note">{occupiedSlots(storage, getItemDef)} 格</span>
        </div>
        <div className="stor-chip">
          <span className="stor-chip-label">装备</span>
          <strong className="stor-chip-value">{gearCount}</strong>
        </div>
        <div className="stor-chip">
          <span className="stor-chip-label">居民积分</span>
          <strong className="stor-chip-value">{loot.toLocaleString()}</strong>
        </div>
      </div>
      ...
      {panel && (
        <div
          className={`stor-modal${closing ? " is-closing" : ""}`}
          onClick={closePanel}
          style={
            {
...
function EntryTile({
  ...
  return (
    <button className="stor-entry" type="button" onClick={onClick}>
      <span className="stor-rim" aria-hidden />
      <span className="stor-entry-icon">{icon}</span>
      <span className="stor-entry-text">
        <span className="stor-entry-name">{name}</span>
        <span className="stor-entry-desc">{desc}</span>
      </span>
      <span className="stor-entry-go" aria-hidden>
        ▸
      </span>
    </button>
  );
...
function PanelHead({
  ...
  return (
    <div className="stor-panel-head">
      <span className="stor-kicker">{kicker}</span>
      <h3 className="stor-panel-title">{title}</h3>
      <button className="stor-close" type="button" onClick={onClose} aria-label="关闭">
        ✕
      </button>
    </div>
  );
...
function StockGrid({
  ...
  if (!shown.length) return <p className="stor-empty">{empty}</p>;
  return (
...
function InventoryPanel({
  ...
  return (
    <>
      ...
      <div className="stor-panel-body">
        <div className="stor-col-main">
          <ItemTabs
            stacks={stacks}
            tab={tab}
            equipTab={equipTab}
          ...
          {sel &&
            (confirming === sel.uid ? (
              <>
                ...
                <button className="stor-btn" type="button" onClick={() => setConfirming(null)}>
                  取消
                </button>
              </>
            ) : (
              <button className="stor-btn" type="button" onClick={() => setConfirming(sel.uid)}>
                丢弃
              </button>
            ))}
        </ItemDetail>
...
function GearPanel({
  ...
  return (
    <>
      ...
      <div className="stor-gear-body">
        {/* 左: 名册 */}
        <div className="stor-roster">
          {awakened.map((id) => (
            <button
              key={id}
              type="button"
              className={`stor-roster-row${id === charId ? " is-on" : ""}`}
        ...
        {/* 中: 三个槽位 + 当前面板 */}
        <div className="stor-slots">
          {EQUIP_SLOTS.map((s) => {
            const worn = cs?.equipped?.[s] ?? null;
            return (
              <div key={s} className={`stor-slot-row${s === slot ? " is-on" : ""}`}>
                <button
                  type="button"
                  className="stor-slot-pick"
                  onClick={() => setSlot(s)}
                  title={`调配${SLOT_LABEL[s]}`}
                >
                  <span className="stor-slot-label">{SLOT_LABEL[s]}</span>
                  {worn ? (
                    <span className={`stor-slot-name r-${getItemDef(worn.itemId).rarity}`}>
                      {getItemDef(worn.itemId).name}
                    </span>
                  ) : (
                    <span className="stor-slot-name is-empty">空槽</span>
                  )}
                </button>
                {worn && (
                  <button
                    type="button"
                    className="stor-btn is-mini"
        ...
        {/* 右: 该槽位的候选装备 */}
        <div className="stor-candidates">
          <span className="stor-section-label">仓库里的{SLOT_LABEL[slot]}</span>
          {candidates.length ? (
            ...
            <p className="stor-empty">仓库里没有可用的{SLOT_LABEL[slot]}。</p>
          )}
        </div>
      </div>
      <div className="stor-panel-foot">
...
function RecyclePanel({
  ...
  return (
    <>
      ...
      <div className="stor-panel-body is-single">
        {sellable.length ? (
          <>
            <p className="stor-note">
          ...
          <p className="stor-empty">没有可回收的物资。废料从远征里带回来。</p>
        )}
      </div>
      <div className="stor-panel-foot">
        <p className="stor-note">
