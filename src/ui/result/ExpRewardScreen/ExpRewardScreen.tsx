...
import s from "./ExpRewardScreen.module.css";
...
export function ExpRewardScreen() {
  ...
  return (
    <div className={cx(s["screen"], s["reward"], s["terminal-screen"], s["center"])}>
      <div className={s["screen-kicker"]}>
        战后处理 / 净化粒子〈{tier.name}〉×{tier.rewardMultiplier.toFixed(2)}
      </div>
      <h2 className={s["terminal-heading"]}>{isBossDone ? "回收总控已停机" : "遭遇已清除"}</h2>
      <p className={s["muted"]}>
        {lastLoot > 0 && (
          <>
            本场缴获 <b className={s["loot-inline"]}>💠 {lastLoot}</b> 居民积分
      ...
      {lastDrops.length > 0 && (
        <div className={s["reward-drops"]}>
          <span className={s["screen-kicker"]}>战利品</span>
          <div className={s["reward-drop-row"]}>
            {lastDrops.map((st) => (
              <ItemSlot key={st.uid} stack={st} />
            ))}
          </div>
          {overflowed && (
            <p className={s["muted"]}>背包装不下 —— 回到路由图后要当场取舍。</p>
          )}
        </div>
      )}

      <div className={s["exp-report"]}>
        {/* ★ 角色没有等级 —— 经验只是进池子, 等玩家拿去训练室锻造卡组。
            所以这里既没有升级提示, 也没有"距下一级还差多少"的进度条。 */}
        {expReport.map((g) => {
          const c = getCharacter(g.charId);
          return (
            <div key={g.charId} className={s["exp-row"]}>
              <CharacterPortrait
                characterId={c.id}
                emoji={c.emoji}
                alt={c.name}
                className={s["exp-portrait"]}
              />
              <div className={s["exp-info"]}>
                <div className={s["exp-name-line"]}>
                  <span className={s["exp-name"]}>{c.name}</span>
                  <span className={s["exp-gain"]}>+{g.gained} EXP</span>
                </div>
                <div className={s["exp-level-line"]}>
                  <span className={s["muted"]}>可用经验 {g.expAfter} · 用于卡组锻造</span>
                </div>
              </div>
            </div>
          );
      ...
      <button className={cx(s["primary"], s["button-big"])} onClick={() => confirmExpReport()}>
