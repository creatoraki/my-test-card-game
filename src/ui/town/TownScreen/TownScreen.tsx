...
//
// 与主菜单同一套「1920×1080 设计画布 + 等比缩放」机制(见 ui/app/shared/stage.ts):
...
//
// ★ 点建筑会播一段「进设施」演出(时长/位移的真相在 ui/town/TownScreen/facilityScenes.ts):
...
const FACILITIES_WITH_OWN_BACK = new Set(["shop", "museum", "assembly", "worklog", "cryo"]);
...
const FACILITY_CONTENT: Record<string, (leaving: boolean, onBack: () => void) => ReactNode> = {
  ...
  // 医疗室: 复苏舱 / 疗养舱 / 圣水池
  cryo: (leaving, onBack) => <CryoScene leaving={leaving} onBack={onBack} />,
  // 研究中心: 模组装配 / 模组制造
