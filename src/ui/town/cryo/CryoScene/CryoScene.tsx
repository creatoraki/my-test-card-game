...
function CryoAmbience() {
  return (
    <div className={cn("cryo-ambience")} aria-hidden>
      <span className={cn("cryo-fog", "cryo-fog-a")} />
      <span className={cn("cryo-fog", "cryo-fog-b")} />
      <span className={cn("cryo-scanlines")} />
      <span className={cn("cryo-motes")}>
        {Array.from({ length: MOTE_COUNT }, (_, i) => (
          // --i 派生出各自的横向位置 / 大小 / 时长 / 延迟(算式在 CSS 里), 免得在 TSX 里
          // 撒一堆 magic number, 也免得每次渲染都重新随机(那会让微粒在重渲染时集体跳位)。
          <span key={i} className={cn("cryo-mote")} style={{ "--i": i } as CSSProperties} />
        ))}
      </span>
    </div>
  );
  ...
  return (
    <div className={cn("cryo-scene", leaving && "is-leaving")}>
      ...
      <header className={cn("cryo-header")} style={{ left: "56px", top: "42px" }}>
        <span className={cn("cryo-kicker")}>CRYOGENIC BAY</span>
        <h2 className={cn("cryo-title")}>冬眠仓</h2>
        <p className={cn("cryo-sub")}>队员唤醒 · 作战编成</p>
      </header>
      ...
      <div className={cn("cryo-readout")} style={{ right: "56px", top: "42px" }}>
        ...
        <div className={cn("cryo-chip")}>
          <span className={cn("cryo-chip-label")}>残片</span>
      ...
      {panel && (
        <div
          className={cn("cryo-modal", closing && "is-closing")}
          onClick={closePanel}
          style={
            {
...
function EntryTile({
  ...
  return (
    <button className={cn("cryo-entry")} type="button" onClick={onClick}>
      ...
      <span className={cn("cryo-entry-icon")}>{icon}</span>
      <span className={cn("cryo-entry-text")}>
        <span className={cn("cryo-entry-head")}>
          <span className={cn("cryo-entry-name")}>{name}</span>
        </span>
        <span className={cn("cryo-entry-desc")}>{desc}</span>
      </span>
      <span className={cn("cryo-entry-go")} aria-hidden>
        ▸
      </span>
    </button>
  );
...
function PanelHead({
  ...
  return (
    <div className={cn("cryo-panel-head")}>
      <span className={cn("cryo-kicker")}>{kicker}</span>
      <h3 className={cn("cryo-panel-title")}>{title}</h3>
      <button className={cn("cryo-close")} type="button" onClick={onClose} aria-label="关闭">
        ✕
      </button>
    </div>
  );
...
function AwakenPanel({
  ...
  return (
    <>
      ...
      <div className={cn("cryo-awaken-body")} style={{ gridTemplateColumns: "440px 1fr" }}>
        {/* 舱位阵列: 2 列网格的**竖向舱体**(编号刻线 → 取景窗 → 状态灯 + 文案),
            比原来的横条更像"一排冬眠舱"。列数在 CSS 里, 不是旋钮。 */}
        <div className={cn("cryo-rack")}>
          {pods.map((pod, i) => (
            <PodCard
        ...
        <div className={cn("cryo-pod-detail")} key={slot}>
          {active?.kind === "sealed" ? (
            <>
              <div className={cn("cryo-sealed-figure")} aria-hidden>
                {/* 缓慢旋转的扫描环: 读作"仪器还在持续采样这具休眠体" */}
                <span className={cn("cryo-scan-ring")} />
                <SealedIcon />
              </div>
              <span className={cn("cryo-kicker")}>DORMANT / {active.charId.toUpperCase()}</span>
              <h4 className={cn("cryo-detail-name")}>休眠体 · 身份未解析</h4>
              <p className={cn("cryo-sealed-desc")}>
                舱盖仍处于密封状态。解封前无法读取该冬眠体的档案 —— 只知道生命体征仍在。
              </p>
              <div className={cn("cryo-vitals")}>
                {VITALS.map((v, i) => (
            ...
            <>
              <div className={cn("cryo-awake-figure", "cryo-vitrine")}>
                <CharacterPortrait
                  characterId={active.charId}
                  emoji={getCharacter(active.charId).emoji}
                  alt={getCharacter(active.charId).name}
                  className={cn("cryo-bust")}
                />
              </div>
              <span className={cn("cryo-kicker")}>RELEASED</span>
              <h4 className={cn("cryo-detail-name")}>{getCharacter(active.charId).name}</h4>
              <p className={cn("cryo-sealed-desc")}>
            ...
            <>
              <div className={cn("cryo-empty-figure")} aria-hidden>
                <NoSignalIcon />
              </div>
              <span className={cn("cryo-kicker")}>NO SIGNAL</span>
              <p className={cn("cryo-sealed-desc")}>此舱位没有冬眠体信号。</p>
            </>
          )}
        </div>
      </div>

      <div className={cn("cryo-panel-foot")}>
        <p className={cn("cryo-note")}>
          {active?.kind !== "sealed"
            ? "选中一个密封舱位才能解封。"
            : affordable
...
function PodCard({
  ...
  const cls = cn("cryo-pod", `is-${pod.kind}`, selected && "is-selected");

  return (
    <button className={cls} type="button" style={stagger(index)} onClick={onSelect}>
      <span className={cn("cryo-pod-lid")} aria-hidden />
      <span className={cn("cryo-pod-frost")} aria-hidden />
      <span className={cn("cryo-pod-brackets")} aria-hidden />
      <span className={cn("cryo-pod-no")}>POD-{String(index + 1).padStart(2, "0")}</span>

      {pod.kind === "awake" ? (
        <>
          <span className={cn("cryo-pod-figure", "cryo-vitrine")}>
            <CharacterPortrait
              characterId={pod.charId}
              emoji={getCharacter(pod.charId).emoji}
              alt={getCharacter(pod.charId).name}
              className={cn("cryo-portrait")}
            />
          </span>
          <span className={cn("cryo-pod-text")}>
            <span className={cn("cryo-slot-name")}>{getCharacter(pod.charId).name}</span>
            <span className={cn("cryo-slot-meta")}>
              <i className={cn("cryo-pod-led")} aria-hidden />
              已解封
            </span>
          </span>
        </>
        ...
        <>
          <span className={cn("cryo-pod-figure", "cryo-pod-icon")}>
            <SealedIcon />
          </span>
          <span className={cn("cryo-pod-text")}>
            <span className={cn("cryo-slot-name")}>休眠体</span>
            <span className={cn("cryo-slot-meta")}>
              <i className={cn("cryo-pod-led")} aria-hidden />
              密封 · 体征稳定
            </span>
        ...
        <>
          ...
          <span className={cn("cryo-pod-text")}>
            <span className={cn("cryo-slot-name", "is-mono")}>NO SIGNAL</span>
            <span className={cn("cryo-slot-meta")}>
              <i className={cn("cryo-pod-led")} aria-hidden />
              空舱
            </span>
...
function VitalCell({
  ...
  return (
    <div className={cn("cryo-vital")} style={stagger(index)}>
      <span className={cn("cryo-attr-label")}>{vital.label}</span>
      <strong className={cn("cryo-vital-value")}>
        {vital.num === undefined
          ? vital.text
