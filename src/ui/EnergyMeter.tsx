...
export function EnergyMeter({ energy, projected }: Props) {
  ...
  return (
    <div className="energy-meter" style={{ ["--energy-color" as string]: cur.color }}>
      <div className="em-head">
        <span className="em-label">净化粒子</span>
      ...
      <div className="em-bars">
        {ENERGY_TIERS.map((t) => {
          ...
          return (
            <span
              key={t.tier}
              className={`em-bar${reached ? " on" : ""}${willReach ? " ghost" : ""}`}
      ...
      <div className="em-foot">
        <span className="em-tier">
          {cur.name} · 产出 ×{cur.rewardMultiplier.toFixed(2)}
        </span>
        {crossing ? (
          <span className="em-warn" style={{ ["--next-color" as string]: after.color }}>
            本段结束后跌入〈{after.name}〉
          </span>
        ) : (
          gap != null && <span className="em-gap">再 −{gap} 跌档</span>
        )}
      </div>
    </div>
  );
