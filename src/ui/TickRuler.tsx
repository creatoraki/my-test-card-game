...
export function TickRuler({ tick, round, enemies }: Props) {
  ...
  return (
    <div className="tick-ruler" title="时刻轴：普通牌推进 1 时刻，速攻牌不推进">
      <span className="tick-ruler-label">
        回合 <b>{round}</b>
      </span>
      <div className="tick-ruler-track">
        {ticks.map((t) => (
          <span key={t} className={`tick-cell${t === tick ? " now" : ""}`}>
            {t}
            {SHOW_ENEMY_MARKS && marks.has(t) && (
              <i className="tick-mark" aria-hidden>
                {"•".repeat(marks.get(t)!)}
              </i>
            )}
          </span>
        ))}
        <span className="tick-cell tail" aria-hidden>
          ▸
        </span>
      </div>
    </div>
