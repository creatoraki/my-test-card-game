// ★ 落点事件面板的外轮廓真相 ★
//
// 事件面板不再是规则四边形: 四条边各带一组「矩形凹陷 / 矩形外凸」, 节奏各不相同。
...
/** 外凸最大伸出量, 同时也是面板四周为外凸预留的边距(元素比内容区四周各大这么多)。 */
export const BUMP = 18;

/** 元素(border-box)尺寸 —— 内容区 + 四周的外凸预留。 */
...
const EDGES: Record<"top" | "right" | "bottom" | "left", Notch[]> = {
  // 上边: 一个长外凸开场, 中段一道浅凹, 收尾一个短促的深凸。
  top: [
    { at: 120, len: 150, depth: 14, dir: "out" },
    { at: 380, len: 90, depth: 12, dir: "in" },
    { at: 620, len: 60, depth: 18, dir: "out" },
  ],
  // 右边: 先凹后凸, 凸块最长, 尾部一道小凹收口。
  right: [
    { at: 90, len: 70, depth: 14, dir: "in" },
    { at: 260, len: 130, depth: 18, dir: "out" },
    { at: 480, len: 50, depth: 10, dir: "in" },
  ],
  // 下边: 凸-凹-凸 交替, 凹段最宽, 读起来最"重"。
  bottom: [
    { at: 100, len: 110, depth: 16, dir: "out" },
    { at: 300, len: 140, depth: 12, dir: "in" },
    { at: 600, len: 80, depth: 18, dir: "out" },
  ],
  // 左边: 小凹起手, 中段一块长外凸, 上部一道深凹。
  left: [
    { at: 80, len: 60, depth: 12, dir: "in" },
    { at: 220, len: 160, depth: 16, dir: "out" },
