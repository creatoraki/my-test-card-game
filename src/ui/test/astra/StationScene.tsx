...
import s from "./StationScene.module.css";
...
export interface StationSceneProps {
  /** 点击（或键盘确认）任意建筑时触发，用于进入子场景。 */
  onEnter: (buildingId: Building["id"]) => void;
}

export function StationScene({ onEnter }: StationSceneProps) {
  const id = useId().replace(/:/g, "");

  const handleKeyDown = (event: KeyboardEvent<SVGGElement>, buildingId: Building["id"]) => {
    ...
    <div className={s.root}>
      ...
      >
        ...
        <desc id={`${id}-description`}>
          将鼠标移到建筑上，或使用 Tab 键聚焦建筑，查看沿建筑外轮廓环绕的光效；点击建筑进入商店场景。
        </desc>
        <defs>
        ...
        {BUILDING_CONTOURS.map((building, index) => {
          ...
          return (
            <g
              ...
              tabIndex={0}
              role="button"
              aria-label={`${building.label}，进入商店场景`}
      ...
      <p className={s.hint}>点击建筑进入商店场景<span> / </span>也可用 Tab 键聚焦后按回车</p>
      <SortieButton />
    </div>
  );
}
