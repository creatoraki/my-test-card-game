...
import { BattleTransitionCurtain } from "./BattleTransitionCurtain";
import { takeTransitionOrigin, type TransitionOrigin } from "./transitionOrigin";
import {
  BATTLE_RIPPLE_MS,
...
import "./screenViewTransition.css";
  ...
  useEffect(() => {
    ...
    const swap = window.setTimeout(() => {
      ...
      if (next.curtain === "battle-ripple") {
        ...
        // 与上面的共享元素分支同一条机制: ScreenTransition.css 里那几条 root 规则已经收窄到
    ...
    <>
      ...
      {spec?.curtain === "battle-ripple" && phase !== "idle" ? (
        ...
        <div
          ...
          key={phase}
          className={`screen-curtain curtain-${spec.curtain}`}
          data-phase={phase}
          aria-hidden
        />
      ) : null}
