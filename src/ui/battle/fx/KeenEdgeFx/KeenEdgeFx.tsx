...
const timing = (milliseconds: number) =>
  `calc(${milliseconds / KEEN_RATE}ms / max(var(--fx-rate, 1), 0.25))`;

export function KeenEdgeFx({ preset }: { preset: ProcFxPreset }) {
  // 组件内部沿 KEEN_TIMELINE 基准轴计算, 下发时由 timing() 统一除以 KEEN_RATE。
  // preset.impactMs 是实播 336ms, 这里换回基准 470ms 以保持几何表的偏移关系。
  const impact = Math.max(preset.impactMs, 0) * KEEN_RATE;
