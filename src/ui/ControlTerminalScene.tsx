...
import "./ControlTerminalScene.css";
...
export function ControlTerminalScene({ leaving = false }: Props) {
  ...
  return (
    <div className={`term-scene${leaving ? " is-leaving" : ""}`}>
      {/* ---- 左上: 场景标题 ---- */}
      <header className="term-header" style={{ left: "56px", top: "42px" }}>
        <span className="term-kicker">CONTROL TERMINAL</span>
        <h2 className="term-title">控制终端</h2>
        <p className="term-sub">城市工单接入 · 下降通道授权</p>
      </header>

      {/* ---- 右上: 两枚读数 chip ---- */}
      <div className="term-readout" style={{ right: "56px", top: "42px" }}>
        <div className="term-chip">
          <span className="term-chip-label">终端积分</span>
          <strong className="term-chip-value">{loot.toLocaleString()}</strong>
        </div>
        <div className="term-chip">
          <span className="term-chip-label">上阵</span>
          <strong className="term-chip-value">
      ...
      >
        <button className="term-entry" type="button" onClick={() => setPanel("descent")}>
          <span className="term-rim" aria-hidden />
          <span className="term-entry-icon">
            <DescentIcon />
          </span>
          <span className="term-entry-text">
            <span className="term-entry-head">
              <span className="term-entry-name">下降舱</span>
            </span>
            <span className="term-entry-desc">出击 · 选定下降层</span>
          </span>
          <span className="term-entry-go" aria-hidden>
            ▸
          </span>
        </button>

        <button className="term-entry" type="button" onClick={() => setPanel("commission")}>
          <span className="term-rim" aria-hidden />
          <span className="term-entry-icon">
            <CommissionIcon />
          </span>
          <span className="term-entry-text">
            ...
            <span className="term-entry-head">
              <span className="term-entry-name">委托</span>
              <span className="term-flag">未接入</span>
            </span>
            <span className="term-entry-desc">接取城市维护工单</span>
          </span>
          <span className="term-entry-go" aria-hidden>
            ▸
          </span>
      ...
      {panel && (
        <div
          className={`term-modal${closing ? " is-closing" : ""}`}
          onClick={closePanel}
          style={
            {
            ...
            {panel === "descent" ? (
              <>
                <div className="term-panel-head">
                  <span className="term-kicker">DESCENT POD / SECTOR SELECT</span>
                  <h3 className="term-panel-title">下降舱 · 目标层选定</h3>
                  <button
                ...
                <div className="term-descent-body" style={{ gridTemplateColumns: "420px 1fr" }}>
                  <div className="term-map-list">
                    {MAPS.map((m, i) => (
                      <button
                        key={m.id}
                        className={`term-map-row${m.id === mapId ? " is-active" : ""}`}
                        type="button"
                        onClick={() => setMapId(m.id)}
                      >
                        <span className="term-map-no">SECTOR-{String(i + 1).padStart(2, "0")}</span>
                        <span className="term-map-name">{m.name}</span>
                        <span className="term-map-stars" title={`难度 ${m.difficulty} / 5`}>
                          {"★".repeat(m.difficulty)}
                  ...
                  <div className="term-map-detail">
                    <div className="term-map-shot">
                      <img src={mapArt(selected.id)} alt="" draggable={false} />
                      <span className="term-map-shot-name">{selected.name}</span>
                    </div>
                    <p className="term-map-desc">{selected.desc}</p>
                    <div className="term-stat-row">
                      <div className="term-stat">
                        <span className="term-stat-label">难度</span>
                    ...
                    <div className="term-party">
                      <span className="term-stat-label">下降小队</span>
                      <div className="term-party-row">
                        {party.map((id) => {
                          const c = getCharacter(id);
                          const cs = characters[id];
                          const s = deriveStats(cs);
                          return (
                            <span key={id} className="term-party-chip">
              ...
              <>
                <div className="term-panel-head">
                  <span className="term-kicker">COMMISSION BOARD</span>
                  <h3 className="term-panel-title">委托 · 城市维护工单</h3>
                  <button
                    className="term-close"
                    type="button"
                    onClick={closePanel}
                ...
                <div className="term-commission-body">
                  {COMMISSIONS.map((c) => (
                    <div key={c.name} className="term-commission is-pending">
                      <span className="term-commission-kind">{c.kind}</span>
                      <span className="term-commission-text">
                        <span className="term-commission-name">{c.name}</span>
                        <span className="term-commission-desc">{c.desc}</span>
                      </span>
                      <span className="term-commission-pay">城市居民积分 ×?</span>
                      <span className="term-commission-state">未接入</span>
                    </div>
                  ))}
                </div>
