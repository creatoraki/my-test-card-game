...
interface Props {
  ...
  large?: boolean; // 战场单位专用的大号血条
  animated?: boolean; // 档案等静态窗口关闭流光、游标粒子与掉血迸溅；战场默认保留。
}

export function HpBar({ hp, hpLimit, maxHp, hideLimit, flush, slowDrain, large, animated = true }: Props) {
  ...
  useEffect(() => {
    ...
    prevHp.current = hp;
    if (!animated || hp >= prev) return; // 静态窗口、治疗、满血重置不触发
    seqRef.current += 1;
    setBurst({
      seq: seqRef.current,
  ...
  return (
    ...
    >
      ...
      <div className={s["hp-fill"]}>
        {animated && <span className={s["hp-flow"]} />}
      </div>
      ...
      {animated && burst && burst.from > burst.to && (
        <div
