// 等距(2:1)投影共享工具 —— 路线图的棋盘、连线、地砖共用同一套斜率。
...
//
// ★ 地砖不是 SVG 画出来的台座, 而是一个**方形 DOM 盒子**被同一个线性变换压平成菱形 ——
//   等距投影本身就是线性变换, 所以「把一个方盒平放到地上」这件事用一条 CSS matrix
//   就是精确的, 多层 background 渐变 / clip-path / mix-blend-mode 的质感一并保住。
