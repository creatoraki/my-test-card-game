...
import s from "./ManaCrystalIcon.module.css";
...
// ⚠ viewBox 是 1:1(0 0 48 48), 调用方改尺寸时必须成对给正方形(见 ManaCrystalIcon.module.css)。
// ⚠ 图内有渐变 <defs>, 而同屏会渲染多颗(顶栏水晶条就是一排), 故用 useId() 保证渐变 id 唯一,
//    否则多实例共享同一 id 会互相串色。off 态的灰暗由 ManaCrystalIcon.module.css 的 filter 负责。
export function ManaCrystalIcon({ className, off }: Props) {
  const uid = useId();
  ...
  return (
    <svg
      className={cx(s["mana-crystal"], off && s["is-off"], className)}
      viewBox="0 0 48 48"
      role="img"
      aria-hidden="true"
      focusable="false"
