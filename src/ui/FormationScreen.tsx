...
//     飞出。编排见 ui/ScreenTransition.tsx 的 viewTransition 分支, 画面见 ui/screenViewTransition.css。
//
...
import { takeSharedPortrait } from "./sharedPortrait";
...
// ⚠ 名字必须全局唯一, 所以只能"一个索引一个名", 而 ui/screenViewTransition.css 里那张错峰
...
export function FormationScreen() {
  ...
  return (
    ...
    >
      ...
      <div className={`screen fm-stage${landingId ? " is-vt-enter" : ""}`}>
...
function CrewCard({
  ...
  // ---- 共享元素过场的命名(规则见 ui/screenViewTransition.css) ----
  ...
  return (
    <div
      className={`fm-card${onField ? " is-on" : ""}`}
    ...
    >
      <button className="fm-card-main" type="button" onClick={onOpen}>
        ...
        <span className="fm-card-figure" style={partName("vt-portrait")}>
          <CharacterPortrait
            characterId={def.id}
            emoji={def.emoji}
            alt={def.name}
        ...
        <span className="fm-card-name" style={partName("vt-char-name")}>
          {def.name}
        </span>
        <span className="fm-card-meta" style={partName("vt-fm-meta")}>
          {Math.round(stats.maxHp)} HP · 攻 {Math.round(stats.attack)} · 卡组 Lv.{cs.deckLevel}
        </span>
        <span className="fm-card-go" style={partName("vt-fm-go")}>
          查看详情 ▸
        </span>
      </button>
      ...
      {onField && (
        <span className="fm-card-flag" style={partName("vt-fm-flag")}>
          出战中
        </span>
      )}
