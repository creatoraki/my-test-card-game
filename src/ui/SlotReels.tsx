...
function SlotCard({ sym, detailed }: { sym: SlotSymbol | undefined; detailed: boolean }) {
  if (!sym) return <div className="slot-card is-blank" aria-hidden />;
  const tags = detailed ? symbolTags(sym) : [];
  return (
    <div className={`slot-card k-${sym.kind}`}>
      <span className="slot-card-bar" aria-hidden />
      <div className="slot-card-art" style={{ backgroundImage: `url(${slotArt(sym.id)})` }}>
        <span className="slot-card-grid" aria-hidden />
        <span className="slot-card-icon" aria-hidden>
          {sym.icon}
        </span>
      </div>
      <div className="slot-card-text">
        <span className="slot-card-kind">{sym.kind === "prep" ? "战前准备" : "战斗"}</span>
        <strong className="slot-card-title">{sym.title}</strong>
        <p className="slot-card-desc">{sym.desc}</p>
        {tags.length > 0 && (
          <span className="slot-tags">
            {tags.map((t) => (
              <em key={t} className="slot-tag">
                {t}
              </em>
            ))}
          </span>
        ...
        {detailed && sym.degraded && <span className="slot-degraded">效果已降级</span>}
      </div>
    </div>
  );
}
...
function Reel({
  ...
  if (picking && stoppedId != null) {
    ...
    return (
      <div className={`slot-reel is-picking${mark}`}>
        <button className="slot-pick" type="button" onClick={(e) => onPick(index, e)}>
  ...
  if (stoppedId != null) {
    ...
    return (
      <div className="slot-reel is-stopped">
        <div className="slot-strip">
          {window.map((id, i) => (
            <div key={`${id}-${i}`} className={`slot-cell${i === 1 ? " is-hit" : ""}`}>
              <SlotCard sym={byId(id)} detailed={false} />
            </div>
          ))}
        </div>
        <div className="slot-marker" aria-hidden />
      </div>
    );
  }
  ...
  return (
    <div className="slot-reel">
      <div
        className={`slot-strip is-spinning${spinning ? "" : " is-paused"}`}
        style={
          {
            "--slot-n": n,
      ...
      >
        {strip.map((id, i) => (
          <div key={`${id}-${i}`} className="slot-cell">
            <SlotCard sym={byId(id)} detailed={false} />
          </div>
        ))}
      </div>
...
function Lever({
  ...
  return (
    <aside className={`slot-lever${disabled ? " is-done" : ""}`}>
      <div className="slot-lever-rail" aria-hidden />
      <button
        className="slot-lever-hit"
        type="button"
        disabled={disabled}
        ...
        aria-label="拉下摇杆停止转轮"
      >
        <span key={pressedKey} className={`slot-lever-arm${pressedKey > 0 ? " is-pulled" : ""}`} aria-hidden>
          <span className="slot-lever-knob" />
        </span>
      </button>
      <div className="slot-lever-base">
        <span className="slot-lever-pips" aria-hidden>
          {Array.from({ length: reelCount }, (_, i) => (
            <i key={i} className={`slot-lever-pip${i < stoppedCount ? " is-on" : ""}`} />
          ))}
        </span>
        <span className="slot-lever-label">{disabled ? "已定住" : "拉下停止"}</span>
      </div>
    </aside>
  );
}
...
export function SlotReels({ onEnterBattle }: { onEnterBattle: (e: MouseEvent<HTMLButtonElement>) => void }) {
  ...
  return (
    <div
      className={`slot-stage${picking ? " is-picking" : ""}${held ? " is-held" : ""}`}
      style={
        {
    ...
    >
      <header className="slot-topbar">
        ...
        {picking && (
          <div className={`slot-match${slot.matchBonus > 0 ? " is-hit" : ""}`}>
      ...
      <footer className="slot-hint">
        <p className="slot-hint-text">
          {held
            ? "战斗条件已确定。"
            : spinning
        ...
        <button
          className="expl-btn is-danger slot-retreat"
          type="button"
